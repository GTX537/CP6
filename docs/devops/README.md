...
| Azure DEV 双模式发布 | 手动 3/3 已验收，自动真实 Run 待验收 | Pipeline/Pool/Variable Group/Environment 均为定向授权，`cp6-dev` 配置 Exclusive lock；#95/#120/#121 三次 Manual 均成功且各有独立备份/证据。自动开关已启用，仍须由新的 main completion 真实执行 Package/Deploy；公网验证保持关闭，SQL 与公网七容器基线门禁不变 |
