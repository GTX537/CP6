...
| Azure DEV 双模式发布 | 外部配置完成，手动验收 1/3 | Pipeline/Pool/Variable Group/Environment 均为定向授权，`cp6-dev` 配置 Exclusive lock；自动 Run #93/#100/#104 安全跳过，手动 Run #95 成功。Run #98/#101/#107 分别证明 Docker 或宿主机重复编译会破坏内存门禁，均在 Deploy 前取消且不计数；DEV 现复用所选 CI 的哈希 Runtime Artifact，只做 Docker 运行时封装，本机真实产物验证保持根环境与 SQL 不变。满三次前不启用自动 |
