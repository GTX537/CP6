...
| Azure DEV 双模式发布 | 外部配置完成，手动验收 3/3 | Pipeline/Pool/Variable Group/Environment 均为定向授权，`cp6-dev` 配置 Exclusive lock；#95/#120/#121 三次 Manual 均成功且各有独立备份/证据。#120/#121 复用 main #118 的同一哈希 Runtime Artifact，仅做运行时封装；备份由 2→4，SQL 与公网七容器基线不变。自动 Run #119 再次证明关闭门安全跳过，是否启用自动仍由用户另行决定 |
