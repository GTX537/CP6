...
版本：v1.3 冻结版（在 v1.2 详细设计上增量修订）
冻结日期：2026-08-12
对应 Epic：E01～E08、E13 的低成本建模与运行验收部分
产品优先级：核心 GA 最高
状态：产品 Spec 冻结；核心 GA 仍须通过自动化、真实 CAD、两仓 Pilot 和生产签字门禁
...
6. [AI 生成、审查与来源追踪](../design/06-ai-generation-review-provenance.md)（外部 AI Provider 按 v1.3 调整为独立 Beta 门禁）
...
`来源留痕 → 规则解析/可选 AI 提案/批量生成 → 人工审查与校正 → Draft 校验 → 发布预览 → CP6 WMS → Published → 生产 Viewer`

低成本不是“任何 CAD 零配置全自动识别”，也不承诺“5 分钟完成数字孪生”；它把工作量集中在图层映射、异常处理和业务确认，避免重复绘制已经能可靠识别的对象。外部 AI Provider 是可选 Beta，禁用或不可用时，规则解析、Excel、底图和手工编辑仍必须完成核心链路。
...
| KPI-09 | CAD 草稿时效 | 50MB 标准 CAD 到可审查草稿/变更集 P95≤15 分钟 |
| KPI-10 | 整体语义准确率 | 20 份授权真实黄金 CAD 的 Validation/Holdout 实测≥90% |
| KPI-11 | 高置信度精确率 | 黄金数据集实测≥95%，但仍须人工确认 |
| KPI-12 | 人工操作量下降 | 作为外部 AI Beta 指标；相对纯人工地图建模≥70%，不阻塞核心 GA |
...
| 自动识别准确率 | `正确自动生成的目标元素数 ÷ 自动生成的目标元素总数 × 100%`，核心 GA 发布门槛不低于 90% |
...
| 2D/3D 一致 | 编辑器导出规范对象清单；Viewer 测试模式从实际 InstancedMesh/对象树导出逻辑 ID、变换矩阵和尺寸清单；两者规范化为整数毫米、RotationZ、编码和逐层规格后计算 SHA-256，哈希必须相同 |
...
标准答案由 E07-S04 数据包提供，必须包含机器可读清单、预期对象数量、几何、业务编码和文件 SHA-256。黄金 CAD 固定按 `Calibration/Validation/Holdout = 10/5/5` 划分；调整规则或阈值只能使用 Calibration，发布结论必须分别报告 Validation 和从未参与调参的 Holdout。修改验收资产时必须提升数据包版本，不能直接覆盖旧答案。
...
以下第一张表是 v1.2 形成时的代码基线，不是 v1.3 目标蓝图；它保留用于解释为什么选择增量收敛，而不是重建领域层：
...
独立门禁：规划与仿真为独立 GA，不阻塞核心 GA；外部 AI Provider 为独立 Beta，不阻塞确定性规则解析；人员调度和设备告警保持 Preview。授权 Demo 模式可以展示模拟人员/设备，但必须默认关闭并持续显示 `Simulated` 标记。

### 5.3 与 E09 外部协作的关系

本文件详细到 E01～E08 和 E13；E09 的组织成员、组合数据范围、字段脱敏和门户由 [Epic 拆分](./03-epic-and-spec-backlog.md) 管理。客户和 3PL 只消费 Published 版本的只读 DTO，不直接读取 Draft、来源文件、解析日志、AI 提案、Lease 或发布重试信息。Supplier 只参加自动化权限/越权矩阵，不参加现场业务 UAT；跨租户猜测和越权测试仍是核心 GA 的硬门禁。
...
flowchart LR
    ...
    PUB --> EXT["标准 WMS 适配器"]
    PUB --> PROD["已发布仓库版本"]
    PROD --> VIEW["生产 Viewer：2D/3D 库存与任务"]
```
...
| 类型 | 租户默认/平台硬上限 | 额外规则 |
|---|---:|---|
| DWG/DXF | 100MB/100MB 每文件 | 100MB 是上传硬上限，50MB 是标准性能验收档；外部参照 XRef 不自动联网解析；缺失参照产生 Blocking；单图≤1,000,000 图元 |
| PDF/PNG/JPG | 100MB/文件 | PDF 最多 200 页；用户必须选择目标楼层页 |
| Excel | 50MB/文件 | 解压后总量≤1GB，压缩比≤20:1 |

租户上限可下调；核心 GA 不允许把 DWG/DXF 上限提高到 100MB 以上。未来若扩容，必须以独立 Scope Change、同规模内存/耗时/安全/费用测试和产品批准为前提。加密文件、宏、嵌入可执行对象和解析器不支持的压缩格式默认拒绝。
...
核心 GA 的“常见设备”固定为：输送线、AGV、叉车、工作台、电子秤和充电桩。它们在本期只提供静态几何、业务编码和自定义属性，不承诺实时状态或运动。
...
`SPACE_TENANT_SCOPE_DENIED`、`SPACE_VERSION_CONFLICT`、`SPACE_VERSION_STATE_INVALID`、`SPACE_SOURCE_UNSAFE`、`SPACE_PARSE_FAILED`、`SPACE_EDIT_LEASE_HELD`、`SPACE_EDIT_LEASE_LOST`、`SPACE_EDIT_LEASE_TAKEOVER_DENIED`、`SPACE_FLOOR_REVISION_CONFLICT`、`SPACE_PARSE_CHANGESET_STALE`、`SPACE_VALIDATION_BLOCKING`、`SPACE_PUBLISH_RECONCILIATION_REQUIRED`、`SPACE_WMS_CAPABILITY_MISSING`、`SPACE_WMS_RETRYABLE`、`SPACE_LOCATION_IN_USE`。
...
决策必须写 ADR。真实 DWG 和 DXF 都是核心 GA 硬门槛。主 Provider 不满足授权、部署或运行条件时必须能切换到已通过同标准审批的备用 Provider；不得把“用户先手工另存 DXF”当作唯一正式方案，也不得用合成 CAD 替代真实文件验收。
...
- 识别置信度 `≥0.90` 进入高置信度分组，可由用户批量选择但仍须显式确认；`0.70～0.89` 生成 Warning；`<0.70` 只生成候选，不直接写语义元素。必需图层存在低置信候选且未处理时为 Blocking。
...
- 上传、单位确认、图层/字段映射。
- 显示后台任务阶段、进度、耗时、取消、失败原因和解析摘要。
- 页面关闭后可从任务中心恢复。
- Job 成功后自动加载待审变更集；确认前 Draft 不变。

### Step 4：校正和编辑

- 左侧模式栏在来源、构件、图层、历史和设置之间切换，并只展开一个上下文面板。
- 中间是可切换的同源 2D/3D 画布。
- 右侧固定属性、批量和问题；逐层货架和来源证据在相应工作域展示。
- 问题点击定位；用户锁定的校正不会被重解析覆盖。
- 顶部始终可见版本、保存状态、撤销/重做、2D/3D、运行校验和“校验并发布”。

### Step 5：3D 预览

- 同一草稿从当前本地场景实时生成，保留选中对象和视角；未保存修改显式标记。
- 可在楼层、全仓和问题对象之间切换。
- 草稿预览不查询或修改生产库存，不显示实时人员或设备；生产 Viewer 只读取 Published。
...
- 核心 GA 选择 CP6 WMS；模拟器只用于自动化和演示，第三方适配器逐个认证后才出现为正式选项。
- 显示绿地/存量模式。
- 发布进度、重试和审计入口可见。
...
### 15.3 核心 GA 可恢复发布 Saga

CP6 WMS、模拟器和第三方适配器都实现同一能力契约，但核心 GA 只认证 CP6 WMS；模拟器不代表生产认证，第三方 WMS 按适配器逐个认证：
...
授权条件还必须包含 TenantId 和 SiteId。Apply 同时要求 `space:model:review-ai` 和 `space:model:edit`；AI 权限不能替代 `space:model:publish`。外部组织用户默认只具有受限 `read`，后端强制拒绝 Draft、来源、AI、Lease、上传和发布端点，不得只通过菜单隐藏。租约接管必须同时检查 `space:model:edit`、`space:model:lease:takeover`、租户/Site/Floor 数据范围和非空原因，并写不可变审计。
...
- Windows 11、4 核 CPU、16GB 内存、Iris Xe 级集成显卡、1920×1080。
...
- 每种问题码数量。
- 人工校正对象比例。
- 规则/AI 提案覆盖率、整体准确率、高置信度精确率、人工修改率和未报告 Blocking 遗漏数。
...
1. 标准 DXF + 标准 Excel 由受训仓库主数据人员在 60 分钟内形成第一次 `Ready`。
2. 真实 DWG 和真实 DXF 都通过主/备用 Provider 合同的同一语义解析流程并得到等价对象；两种格式都是硬门槛。
...
11. 2D、3D 和机器可读对象清单的 LogicalId、数量、尺寸、编码和逐层规格 100% 一致。
...
31. 至少 20 份经授权的真实黄金 CAD 覆盖 5 类仓库布局，并固定按 Calibration/Validation/Holdout = 10/5/5 纳入版本化验收。
...
42. 核心 GA 硬拒绝超过 100MB 的 CAD 文件和超过 100 万图元的 CAD；单租户第 4 个并发 AI 任务被拒绝。
...
- 后台任务状态不能通过猜测 ID 读取。
- 公共模板不可被租户修改。
- 外部用户对 AI、Draft、Source、Lease、Upload、Validate 和 Publish 端点全部拒绝；跨租户 Run/Proposal/Lease ID 猜测拒绝。
- `MetadataOnly/StructuredFeatures` 外发快照不包含原文件、密钥和禁用字段。

### 20.4 端到端测试

- DWG+Excel 和 DXF+Excel 完整路径。
- 底图+编辑器和空白画布完整路径。
- 两条路径统一结果比较。
- 绿地 WMS 发布和存量 WMS 采纳。
- 发布失败、重试、回退。
...
固定自动化测试资产必须包含规范样本、非标准样本、损坏文件和带业务错误的 Excel，且不含未授权客户数据。真实黄金 CAD 必须经过授权、隔离和访问审计；评估包至少包含 20 份真实 CAD、5 类布局、10/5/5 划分、允许外发的最小化 IR、机器可读标准答案和版本化阈值报告。
...
2. CAD+Excel、底图+编辑器、空白模板同时属于核心 GA。
...
4. T4：外部 AI Beta 只处理结构化 DWG/DXF CAD IR；PDF/图片视觉识别和自然语言生成后置。
