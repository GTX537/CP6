...
| Design V1 Contracts/Domain/Application | Partial | `539d56de` + `85792161` + `36f534d9` + `2ccdff7a` + `6e67a9d1` + `6d751e0c` 已集成 E01 S01–S06 与 E07 S01–S04 的版本、来源文件、Job Ledger、Clone、Design API、SDK、文件安全、WMS 合同和标准仓数据集；其余用例仍待逐项进入基线 |
...
| CP6 WMS 适配器、标准模拟器与发布 Saga | Partial | `d06a8bd1` + `6e67a9d1` + `74577015` + `6d751e0c` 已集成 E07 S01–S04 的真实适配器、幂等账本、标准模拟器和确定性 10,000 库位数据集；E07 S05 与 Design V1 可恢复外部 Saga 尚未完成 |

候选工作树不是实现真相。`0d25da4d` 只是保全检查点；只有经过 ADR-0003 的文件审计、按子任务独立提取、迁移验证和测试后，才能更新为 Implemented。当前唯一 Space 代码集成基线是 `6d751e0c`；E02 S01 仍是 Partial，不得以实验工具替代最终技术选型或生产适配器。
