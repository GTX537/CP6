...
功能分支先推送远端备份并验证本地/远端 tip 均为 `034a1b1b`。受控集成与合并态
冒烟完成后，确认该 tip 是本地/远端一致的集成 tip `2514ab56` 的祖先，再删除功能
工作树及本地/远端临时分支。测试用 `node_modules` Junction 只删除链接，共享依赖目标
仍存在。D 盘可用空间由 23,763,136,512 字节增至 25,950,846,976 字节，本轮释放
2,187,710,464 字节（约 2.04 GiB）。
