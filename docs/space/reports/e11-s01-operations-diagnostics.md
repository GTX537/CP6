...
功能分支先推送到远端形成备份；在证明功能 tip 与远端一致、是受控集成 tip 的祖先，
且集成本地与远端一致后，已删除功能工作树及本地/远端临时分支。两个测试用依赖
Junction 只删除链接，底层依赖目录继续保留。D 盘可用空间由 22,735,712,256 字节
增至 24,912,179,200 字节，本轮释放 2,176,466,944 字节（约 2.03 GiB）。
