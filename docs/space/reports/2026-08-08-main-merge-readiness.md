...
当前仍未修改、推送或合并远端 `main`。下一步是把本地 Docker 修复 `9ffbf8f4` 作为独立提交带入候选，提交并推送候选分支，再由受保护 PR 和生产备份恢复副本演练决定是否进入 `main`。
...
1. 形成并提交当前 merge candidate。
2. 独立带入 Docker HTTP Cookie 修复 `9ffbf8f4`，确认只影响本地 Docker 配置。
3. 推送候选分支并运行 GitHub PR 门禁、代码审查和迁移人工复核。
4. 在生产备份恢复副本重跑数据库包两轮并留存证据。
5. PR 与部署审批通过后才合并 `main`；标签、R2 candidate 和生产部署均为独立后续动作。
