...
## 2026-09-08 CRM 身份桥本地实现与安全验证

- [身份与目录桥](../crm/CRM-OIDC-FIRST-SLICE.md) 已完成任务分支代码及安全复核后的本地 97 项相关测试、22 项真实 SQL 门禁和前端检查；包含精确客户端/回调、一次性 PKCE code、真实单租户权限和全局退出。真实 refresh 服务已验证退出竞态、其他浏览器/原生会话隔离、70 次轮换与原认证版本不可升级，真实重用检测保持生效。全新隔离 fixture 的 5 组真实跨仓浏览器流程也已通过，含在线撤销 PII 与旧页面清理；远端提交和必需 CI 状态另行核对。
- 本条记录本地实现证据，跨仓集成、远端 exact-main 与业务验收继续跟踪，不能读作 C01/C02 整体或生产完成。

## 2026-09-08 CRM 首片文档登记准备

- 为本次已授权且已审查的首片 PRD/spec 增补建立独立准备分支；公开 `docs/crm` 仍为历史文档集合，没有在此发布新增说明。
...
- 本地集合/触发配置检查 74/74、审批及敏感内容回归 59/59 通过；`pwsh -NoProfile -File tools/Test-CrmV1Prd.Tests.ps1` 可复现两组检查。旧受保护校验器验证准备分支、新校验器验证已审查首片源，均通过 `-VerifyGitHubEvidence`。这只是登记准备，正式合入与后续实现文档发布仍按受保护分支流程完成，不改变 M0 或生产授权。
