...
- 合并前审查移除商业 cohort 数量/地域/名单、精确推广计划和私有数值商业 KPI，并从最新 `main` 建立不继承旧敏感提交祖先的干净候选分支。
- 两次未合并的预审批尝试因脱敏声明不完整而作废；当前候选仍需唯一 ProgramOwner 对精确摘要和五项产品结论重新批准。M0 继续 No-Go；未创建业务代码、云资源、Secret、数据库、迁移、候选制品或部署，也未解锁 CRM01。
