...
- 详细 Spec LM-FR-017 的 CAD 异常对象“拆分”已完成可逆 Design V1 纵切：单个 Active 非资产组合可拆成 2–100 个独立元素，首部件保留当前 LogicalId，其余部件分配新 LogicalId并继承类型、父级、业务链接、设计属性和 CAD 来源。组合整体移动/旋转会按渲染器同一局部到世界变换展开；保存/撤销/重做分别使用 Update+Create、Update+Delete、Update+Restore，重做不重复创建。2D/3D 等价、真 SQL 原子与零写入、工作台确认和 Playwright 已覆盖。LM-FR-017 当前仅缺画布重画，WP4 继续 Partial，核心 GA 仍为 72% / `NoGo`。
- 详细 Spec LM-FR-017 的 CAD 异常对象“合并”已完成一条可逆 Design V1 纵切：2–20 个同类型、同父级、同业务链接和同设计属性的 Active 非资产元素可合并，保留首选 LogicalId，其余元素标记删除；`group` 组合几何逐部件保存来源、位置、旋转、尺寸和原始几何，并以 100 部件/8 层上限失败关闭。2D/3D 同源渲染、聚合选择边界、工作台确认、撤销/重做、真 SQL 原子批和 Playwright 已覆盖。拆分已由上条关闭，LM-FR-017 当前仅缺画布重画，WP4 继续 Partial，核心 GA 仍为 72% / `NoGo`。
- 详细 Spec LM-FR-017 的 CAD 异常对象“改类型”已完成一条 Design V1 纵切：通用元素在同一 `UpdateProperties` 命令内改型，保留 LogicalId，并继续受 Lease、Floor/Content Revision、幂等与原子审计保护；资产实例禁止改型。工作台提供受支持类型选择，并以补偿命令支持撤销/重做。真 SQL 1/1、Space Unit 全量 526/526、OpenAPI 44/44、前端全量 780/780、类型检查和 Space Studio Playwright 全量 15/15 通过。该切片复核时把 WP4 从过宽的 Complete 校正为 Partial；后续合并与拆分已由上两条关闭，当前仅缺重画，核心 GA 仍为 72% / `NoGo`。
