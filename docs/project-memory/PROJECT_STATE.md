...
- 功能 tip 已先远端备份并确认进入远端集成祖先链；临时分支/工作树将在本状态记录推送后清理，`main` 未修改。
