...
## CRM 首个切片身份桥：任务分支实现（2026-09-08 UTC）

- 全新隔离环境的跨仓真实浏览器联合验证已完成 5/5：实际 Vue 密码登录/OIDC、Next/BFF/SQL 业务链、回执归属、作用域/遮罩与全局退出、实际撤销 PII 权限后的旧页面清理全部通过。使用合成用户与业务数据，远端 PR/main 仍待核对，不扩展为真实客户或生产验收。
- CP6 新增默认关闭的 RS256 OIDC 授权码/S256、显式真实租户开通映射、实时 CRM 权限/部门/负责人投影及绑定原浏览器 refresh 链的全局退出。精确协议、配置和当前验证见 [身份桥记录](../crm/CRM-OIDC-FIRST-SLICE.md)。
- 安全复核后 97 项相关 .NET 测试、22 项真实 SQL 门禁通过；持久化浏览器会话认证版本与退出标记修复旧 refresh 在 2FA 变化后升级、并发退出波及其他会话及超过 64 次轮换无法退出的问题。前端安全返回测试、type-check 与 build 已通过；两项前向认证迁移、模型快照和 PR/main SQL 门禁一并交付。跨仓浏览器与业务验收、远端 exact-main 确认仍由本次集成交付继续完成，当前记录不声称 C01/C02、商业 Provisioning、Pilot 或生产验收完成。

## CRM 首片公开文档有限预登记（2026-09-08）

- 本次明确授权实施首片并同步 PRD/spec，已审查文档对应源提交 `b92e0a23f55e83ba92b6755b5381ef7d0b1e1e27`。本准备仅登记这一批确定内容的完整文档集合；`docs/crm` 仍保留原始 12 份文档，新文档尚未在本准备中发布。
...
- 先让准备分支通过既有 protected-base 门禁并合入受保护 `main`，后续实现分支再同步该基线并提交已登记文档；main push 路径过滤补入新模块及测试，required context、受保护验证步骤、工作流权限、部署和其他业务保持原样。
