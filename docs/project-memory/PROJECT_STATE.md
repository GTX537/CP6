...
- `docs/crm/CRM-V1-PRD.md` v0.2 已迁移到最新 `main` 基线，并移除商业 cohort 数量/地域/名单、精确推广时间表和私有数值商业 KPI；候选期措辞改为不随状态漂移的批准规则，候选 payload SHA-256 为 `3edd1fa3ed841376124c18a2f81d805a48e57870f77832fc44dad6cde3fd94ec`。
- 两次未合并的预审批尝试因脱敏声明不完整而作废；相关证据仅保留在未合并 PR 审计轨迹中，新的干净分支不继承旧敏感提交祖先，也不把旧对象引用写入公开产品基线。
