...
- WP3 Preparation → Parse 的确定性映射输入已补齐：Ready Preparation 保存服务器生成、规范 JSON、SHA-256 密封的 Mapping Replay Snapshot，绑定 Tenant/Source/不可变 Profile、Inventory/Structure/Preview Hash 与完整 Layer Overrides；新 Parse payload v4 携带同一快照，启动服务和 Worker 分别在入队和 Provider 调用前验证。缺失/篡改 v4 快照零 Job 或零 Provider 调用，历史 v2/v3 显式兼容。该项只为真实适配器提供重放合同；真实 Provider 仍须加载冻结 Profile、执行覆盖重放和 `ValidateReplay`，因此 WP3 仍为 Partial/Pending。
- CAD Preparation/Parse 现在统一经 `SpaceCadProviderRouter`：只选择当前 Site 已认证、有效、格式匹配且在当前部署注册的 Provider；Primary 仅在可重试资源故障时切到同配置 Backup，不会跨到未认证链，Preparation 已由 Backup 密封时也不会反向切回 Primary。Preparation 保存 Provider 身份，Parse payload v4 绑定 Provider Key、Semantic Preview Hash 和 Mapping Replay Snapshot；旧 v3/v2 仍可按当前 Site 合规链读取，v1 继续拒绝。
- Space Studio CAD 起始向导先读取 Site CAD 能力，展示配置 Revision、主备链和阻断码；没有有效运行链时禁止扫描轮询和 Preview。`CanPrepareCad` 只代表当前至少一条资格完整、已批准且运行可用的链可用于内部准备，`CadGaReady` 只有主备两条链资格完整、同评测基线、排名有效、认证有效、运行可用且同时覆盖 DWG/DXF 才成立。
...
- WP5 生产 Viewer 的仓库权威边界已闭环：新增 Site 级 `published-scene` 只读合同，只从 `CurrentPublishedVersionId` 解析 Production/Published Design Revision，且明确排除 runtime overlay；单层 Viewer、跨层 Viewer、Control Tower 与楼层列表均不再读取可变旧 `/space/floor/*/scene`/floor API。客户端再次绑定 Site/PublishedVersion/状态并按 RackLevel 确定性投影可拾取 Location，Draft 注入、身份漂移、缺失几何或跨层任一楼层失败均整体失败关闭，不留下半仓画面。OpenAPI、双 SDK、权限和结构守卫已同步；真实 SQL 用例已编译但本机因未配置 `CP6_TEST_SQLSERVER` skipped，生产等价部署、真实发布数据和独立 QA 复核仍是门禁。
...
- 当前累计仓库门禁：完整 Release solution 0 warning / 0 error；Space Unit 512/512、CP6.Tests 2,916 passed / 19 environment-gated skipped、Client 71/71、Space Integration 313 passed / 106 SQL/environment-gated skipped、Web 775/775、Published Viewer 聚焦 12/12、Viewer 性能聚合单测 5/5、Viewer CPU 性能 1/1、Iris Xe 正式冷运行 30/30、发布管理聚焦 5/5、恢复指标/告警合同 6/6、外部主体矩阵 30/30、权限/OpenAPI/主体边界聚焦 112/112、GA 证据索引 2/2、Vue type-check、production build、OpenAPI/C#/TypeScript SDK drift、EF pending-model 和 diff whitespace 通过。新增 Provider 资格与 Published/Draft 隔离等 SQL 用例因本机未配置 `CP6_TEST_SQLSERVER` 而 skipped，不计为真实 SQL/WMS 证据；合并后仍执行聚焦冒烟。
