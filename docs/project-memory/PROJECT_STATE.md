...
- LM-FR-012 的 CAD 来源结构审核已接入起始向导：Design V1 Preparation Preview 在不暴露原始 CAD 字节、块引用明细或外部引用路径令牌的前提下返回完整图层/块清单，工作台可搜索并查看图层名称、颜色、线型、可见性、对象/支持/未支持计数，以及块定义、引用和属性引用计数。用户可逐图层选择沿用 Profile、忽略或覆盖目标，并显式调整几何规则和置信度；单位、坐标、Profile 或 Override 变化都会使旧 Preview 失效，必须重新由服务端密封后才能启动 Parse。系统/租户 Scope 在选择器中明确标识，但当前默认目录仍只有 System Profile，因此 LM-FR-013 仅关闭逐层 Override 部分，租户私有 Profile 持久化/管理仍待后续。真 SQL Space Integration 447/447（0 skipped）、Space Unit 540/540、CP6.Tests 2,932、Web 863、OpenAPI/双 SDK、类型检查、生产构建和完整 solution Release 0 warning / 0 error 通过。WP4 继续 Partial/Pending，核心 GA 保持 72% / `NoGo`。
...
- Space Studio CAD 与 PDF/图片底图上传已显式消费服务端 `Reused`：只有后端按 SHA-256 确认复用时才提示用户已复用受控文件或当前来源，新文件仍显示普通成功提示；底图复用后继续走同一 Clean/Scanning/Rejected 与挂接链。客户端不计算权威哈希、不绕过隔离扫描。Excel 后端/SDK 已有 Reused 合同，但统一 Excel 上传向导仍缺。WP4 继续 Partial/Pending，核心 GA 保持 72% / `NoGo`。
