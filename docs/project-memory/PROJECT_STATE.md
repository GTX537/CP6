...
- 详细 Spec LM-FR-017 的 CAD 异常对象“合并”已完成一条可逆 Design V1 纵切：2–20 个同类型、同父级、同业务链接和同设计属性的 Active 非资产元素可合并，保留首选 LogicalId，其余元素标记删除；`group` 组合几何逐部件保存来源、位置、旋转、尺寸和原始几何，并以 100 部件/8 层上限失败关闭。2D/3D 同源渲染、聚合选择边界、工作台确认、撤销/重做、真 SQL 原子批和 Playwright 已覆盖。LM-FR-017 当前尚缺拆分与画布重画，WP4 继续 Partial，核心 GA 仍为 72% / `NoGo`。
- 详细 Spec LM-FR-017 的 CAD 异常对象“改类型”已完成一条 Design V1 纵切：通用元素在同一 `UpdateProperties` 命令内改型，保留 LogicalId，并继续受 Lease、Floor/Content Revision、幂等与原子审计保护；资产实例禁止改型。工作台提供受支持类型选择，并以补偿命令支持撤销/重做。真 SQL 1/1、Space Unit 全量 526/526、OpenAPI 44/44、前端全量 780/780、类型检查和 Space Studio Playwright 全量 15/15 通过。该切片复核时把 WP4 从过宽的 Complete 校正为 Partial；后续合并已由上条关闭，当前仍缺拆分和重画，核心 GA 仍为 72% / `NoGo`。
