...
- GA 总索引已更新到 2026-08-15，仓库完成度审计登记为 WP0 实现证据，AutoCAD 开发转换报告登记为 WP3 实现证据；两者没有进入 `acceptedEvidence`。证据 JSON 兼容层修复 PowerShell 7.6 自动把 ISO 时间转换为 `DateTime` 后合法 UTC 证明被误拒的问题，PowerShell 5.1/7.6 仍执行同一严格字符串、UTC 和未来时间校验。审计仍判定 WP0/WP3 为 Partial、WP7/WP8 为 ExternalExecution，其余核心工作包仅为仓库实现 Complete，所有接受状态继续 Pending。
