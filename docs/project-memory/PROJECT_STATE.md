...
- Space Studio CAD 与 PDF/图片底图上传已显式消费服务端 `Reused`：只有后端按 SHA-256 确认复用时才提示用户已复用受控文件或当前来源，新文件仍显示普通成功提示；底图复用后继续走同一 Clean/Scanning/Rejected 与挂接链。客户端不计算权威哈希、不绕过隔离扫描。Excel 后端/SDK 的 Reused 合同继续由服务器决定；当时缺失的当前 CAD + Excel 上传向导已由同日后续纵切补齐，历史 CAD 候选目录仍待实现。WP4 继续 Partial/Pending，核心 GA 保持 72% / `NoGo`。
