...
- WP7 正式黄金 CAD 已建立结构化证据门禁：恰好 20 份唯一真实授权样本、10/5/5、L1～L5≥4、DWG+DXF、双标注/独立 QA 仲裁、冻结版本/Worker/Source Set、Primary/Backup 两份 release-eligible 报告、主 Provider 严格高分、两者使用同一 50 MiB 标准 CAD、Overall/Out-of-sample 五项质量指标、Holdout 零未报告 Blocking 以及 50 MiB/Ready P95 都进入同一 Manifest。WP7 Accepted 前还必须完成授权 CAD、Provider/Worker 两类外部输入和 WP3 验收，并由 Gate 证明 Manifest 自身哈希；模板/fixture/原始 CAD 路径均拒绝。专项 31/31、组合证明链 29/29。该项只关闭证据格式和误报，真实 20 CAD 与 Provider 运行仍未发生，WP7 保持 `ExternalExecution/Pending`。
