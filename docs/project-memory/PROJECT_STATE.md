...
- Space Studio CAD 与 PDF/图片底图上传已显式消费服务端 `Reused`：只有后端按 SHA-256 确认复用时才提示用户已复用受控文件或当前来源，新文件仍显示普通成功提示；底图复用后继续走同一 Clean/Scanning/Rejected 与挂接链。客户端不计算权威哈希、不绕过隔离扫描。Excel 后端/SDK 已有 Reused 合同，但统一 Excel 上传向导仍缺；LM-FR-005 来源删除预检也未实现。WP4 继续 Partial/Pending，核心 GA 保持 72% / `NoGo`。
