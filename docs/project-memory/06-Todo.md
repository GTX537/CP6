...
- AutoCAD 2025 Core Console 候选已在远程隔离 Worker 内覆盖 DWG/DXF，托管 Parser 也已用 64 MiB 上限通过精确 50 MiB 合成容量包络；组合 Provider Version 当前绑定 `{core}+cp6-dxf-1.1.0`。它仍不是已认证 Site Provider：下一步须用授权真实 50 MiB DWG/DXF 记录 P95/资源/质量，形成非 `development` Release，确认许可证边界，证明真实 mTLS/禁网/专用身份/临时盘/清除失败关闭并取得客户/Site 批准。GUI `acad.exe` 的 `HashMismatch` 仍应通过 Autodesk 修复或批准例外处理；候选只调用签名有效的 Core Console。未完成前不得填写 `acceptedEvidence` 或关闭 WP3/WP7。
