...
- #95/#120/#121 手动 3/3 与 #125 真实 `ResourceTrigger` 自动发布均已完成；自动链已核对 Package、备份/VERIFYONLY、Deploy、健康/身份、证据 Artifact 和根环境零漂移。`CP6_DEV_AUTO_DEPLOY_ENABLED=true` 继续生效；任何旧版本手动回退前必须先关闭自动。
...
- 三次手动 Run 与 #125 真实自动 Run 均已完成并证明根 `cp6`/`CP6DB` 未受影响；自动开关保持 `true`。后续每次自动发布继续保留备份/部署证据与宿主基线门禁。
