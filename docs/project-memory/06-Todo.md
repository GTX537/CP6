...
- P08 已完成 S00-S05：`0.8.0-alpha.2` 不可变发布、CRM 固定版本黑盒消费、证据前向纠正、Platform 对账以及公共项目记忆同步均有精确 PR/head/merge/run/artifact/hash 记录；当前必须保持 `Published / Consumer Candidate`。
- 下一步只执行 P08-S06 Platform 最终证据审计。只有最终 PR 合并且 exact-main Windows/Linux/真实 Dapr-Kafka/真实 SQL 四项门禁成功后，才可首次把 Platform、CRM locator 和公共项目记忆依次传播为 `Frozen / Consumable`；在此之前不得推进 P09 或把候选状态写成冻结。
