...
- 首次手动 `cp6-dev` 已由 Run #95 成功发布，但自动验收仅完成 1/3。Run #98 因 Docker 构建 OOM 在 Deploy 前取消且导致根 `cp6-db`/`cp6-api` 自动重启，明确不计数。低内存 publish 修复合入后，继续保持 `CP6_DEV_AUTO_DEPLOY_ENABLED=false`，再完成两次有独立备份/证据的成功 Manual Run；每次运行前确认宿主 `KOUSQLSERVER` 可执行真实查询且没有新 701/17300，并记录根七容器 ID、StartedAt 与 RestartCount 前后不变。
