...
- 以 `f9c7fd21` 为当前 Space 代码集成基线。E04 S01–S04、E05 S01–S05、E13 S01–S03/S12 均已完成，不要重复实现。
- E04 S04 已交付货架/通用元素统一多选、对齐、等距、旋转、批量删除、阵列和保存后补偿式撤销/重做；继续保持 Draft/revision、全批次原子性、协议幂等和逐命令审计，不得把 WMS 运行态或 AI 生成混入编辑器。
- 下一张建议卡为 E07 S05 存量 WMS 采纳与绑定；其 E07 S02 + E04 S04 依赖已满足，应完成拉取、放置、绑定、未绑定和差异对账闭环。S01 的默认扫描器仍失败关闭，生产启用前必须配置真实扫描引擎和共享耐久文件卷。
...
- E07 S01–S04 已完成；标准仓数据包已可确定性重建 500 货架、10,000 库位、库存、任务和异常场景。E07 S05 的 E04 S04 前置依赖已解除，可按独立卡推进。
- E13 后续仍须保证原始 CAD 不外发；Mock、本地和外部适配器只能在 S05 使用 S01 的同一端口实现，规则路径不得依赖外部 Provider。
- 本机 `KOUSQLSERVER` 的 Windows 集成认证已可用于提权测试。E04 S02 文件安全、标定 Migration 与 Clone 聚焦为 9/9 passed，E04 S03 命令闭环为 1/1 passed，E05 S01–S04 与受影响 Clone 的真实 SQL 串行链为 11/11 passed；默认门禁仍保留环境隔离 skip 语义，不能把未启动的 SQL-gated 测试写成通过。
