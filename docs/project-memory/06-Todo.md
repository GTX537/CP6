...
- WP4 详细 Spec LM-FR-017 仍缺 CAD 异常对象合并、拆分和画布重画：按独立任务分支依次补 typed command、几何/身份规则、Lease + Revision + 幂等 Fence、原子审计、撤销/重做和真 SQL/Playwright 证据。改类型与删除已具备，但不得据此把 WP4 标回 Complete；三项均闭环后再复核其它未实现 LM-FR 条目。
...
- WP4 的图片底图标定入口、Excel–CAD 深链审核/确认、DWG/DXF 分格式浏览器合同及异常对象改类型/删除已有仓库内自动化；异常对象合并、拆分和重画仍缺，因此实现状态为 Partial。实现闭环后还须用授权真实 DWG、DXF、Excel 和 PDF/图片在两条已认证 Provider 链及 CP6 WMS 环境完成端到端证据。Mock/fixture 结果不得计入黄金 CAD、性能、恢复或 Pilot 完成度。
