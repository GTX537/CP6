...
- WP4 详细 Spec LM-FR-017 的改类型、删除、合并、拆分、画布重画以及 LM-FR-023 的对齐、等距、复制、旋转、阵列已有仓库实现。下一独立任务优先关闭 LM-FR-018：重新解析前展示会替换的自动生成对象，并以持久化锁定/Apply Fence 保护人工校正；随后继续逐项复核其它 LM-FR 与三条路径。上述仓库闭环不自动把 WP4 标回 Complete，也不得用 Mock/fixture 替代外部接受。
