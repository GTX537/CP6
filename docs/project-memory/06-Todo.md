...
- AutoCAD 2025 Core Console 候选已在远程隔离 Worker 内覆盖 DWG/DXF：DWG 经 Core Console + 托管 Parser，原生 DXF 不启动 AutoCAD；组合 Provider Version 同时绑定两侧版本，完整源 SHA、双层合同执行、CAD IR-only 响应和 Attempt 清理均通过。它仍不是已认证 Site Provider；下一步须形成非 `development` Release，解决托管 DXF Parser 当前 25 MiB 上限与 50 MiB 正式场景，确认许可证边界，证明真实 mTLS/禁网/专用身份/临时盘/清除失败关闭并取得客户/Site 批准。GUI `acad.exe` 的 `HashMismatch` 仍应通过 Autodesk 修复或批准例外处理；候选只调用签名有效的 Core Console。未完成前不得填写 `acceptedEvidence` 或关闭 WP3/WP7。
