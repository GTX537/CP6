...
- Layout Command 创建链已在独立任务分支实现；下一张任务卡接入 Space Studio 的 Zone/Aisle/Rack 创建表单/拖放，并补齐设计态修改、删除和级联语义。随后单独实现批量编码 Preview → Apply，禁止调用旧运行态服务直接改 Published/WMS；旧 `FloorEditor` 不继续发展为第二套权威。
