...
- 以 `39146c38` 为当前 Space 代码集成基线，`407dcbea` 为对应文档基线。E04 S01–S03、E05 S01–S05、E13 S01–S03/S12 均已完成，不要重复实现。
- 下一张可独立推进的 3D Space 卡为 E04 S04：多选、对齐、分布与阵列命令。必须扩展 S03 的选择模型和命令批次，保持 Draft/revision、全批次原子性、协议幂等与逐命令审计；不得把 WMS 运行态或 AI 生成混入编辑器。
- E04 S04 完成后才解除 E07 S05 的采用前置条件。S01 的默认扫描器仍失败关闭，生产启用前必须配置真实扫描引擎和共享耐久文件卷。
