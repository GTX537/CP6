...
- #95/#120/#121 已完成 3/3，用户已授权且 Azure `CP6_DEV_AUTO_DEPLOY_ENABLED=true`。仍须由新的 main completion 触发一次真实自动 DEV Run，核对 `ResourceTrigger`、Package、备份/VERIFYONLY、Deploy、健康/身份、证据 Artifact 和根环境零漂移；#124 的同 SHA 手动基础 CI 重跑不算自动验收。旧版本手动回退前必须先关闭自动。
...
- 三次手动 Run 已完成并证明根 `cp6`/`CP6DB` 未受影响；自动开关已获授权并设为 `true`。下一步以新 main completion 完成一次真实自动发布验收，并持续保留每次备份/部署证据与宿主基线门禁。
