...
## CRM 首个业务切片集成（2026-09-08 UTC）

- [身份桥契约](../crm/CRM-OIDC-FIRST-SLICE.md) 的本地真实 BFF/业务浏览器联合验证已完成 5/5，继续核对两仓库审计提交与 PR/main 必需门禁；旧会话需重新登录后获得浏览器链绑定。
- 在目标测试环境由真实目录选择 TenantId、slug/region/CRM entitlement，注入 RSA/机密客户端配置，执行授权账本与持久化浏览器会话两项前向迁移，保留精确源码绑定的远端 SQL/TRX 证据。安全修复本地 97 项相关测试与 22 项真实 SQL 已通过，独立复核及跨仓真实登录仍须完成；不能将旧 refresh 轮换计为重新认证。商业 Provisioning、C01/C02 全架构、Pilot/UAT/生产批准仍各自按原门禁关闭。

## CRM 首片文档登记后续（2026-09-08）

- 有限预登记准备只保存可信校验规则与回归，公开文档仍为历史集合；准备分支须先通过原有 protected-base 门禁、合入受保护 `main` 并验证远端精确提交。
- 首片实现分支随后同步已保护的规则，再提交源 `b92e0a23f55e83ba92b6755b5381ef7d0b1e1e27` 中已登记的四份文档变更，完成实现、PR 与合并后验证。内容有任何后续变动均须重新审查登记，不能放宽摘要或路径检查。
- 此次登记不关闭真实组织接入、商业 Provisioning、Pilot、发布或生产门禁；历史产品批准和 M0 No-Go 保持原约束。
