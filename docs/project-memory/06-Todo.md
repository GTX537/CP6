...
- AutoCAD 2025 Core Console 的实验型 DWG→DXF→CAD IR 链已在本机通过，但它不是已认证 Site Provider：下一步需先通过 Autodesk 更新/修复处理 GUI `acad.exe` 的 `HashMismatch`，再确认许可证允许的 Worker/自动化部署边界、禁网或出口控制、客户/Site 批准，并与至少一个独立备用 Provider 在同一 20 份真实黄金集上评分。未完成这些条件前不得注册生产运行时或关闭 WP3/WP7。
