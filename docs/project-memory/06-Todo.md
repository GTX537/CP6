...
- 以 `c1043d15` 为当前 Space 代码集成基线，`96113ea3` 为对应文档基线。E04 S01–S02、E05 S01–S05、E13 S01–S03/S12 均已完成，不要重复实现。
- 下一张可独立推进的 3D Space 卡为 E04 S03：通用元素选择与属性面板。必须复用统一 Design Scene 和既有通用元素领域模型，让墙、柱、门等草稿元素可选择、编辑和删除；不提前混入 S04 的多选/对齐/分布/阵列。
- E04 后续固定为 S03 通用元素选择/属性面板 → S04 多选/对齐/分布/阵列；S04 完成后才解除 E07 S05 的采用前置条件。S01 的默认扫描器仍失败关闭，生产启用前必须配置真实扫描引擎和共享耐久文件卷。
...
- 本机 `KOUSQLSERVER` 的 Windows 集成认证已可用于提权测试。E04 S02 文件安全、标定 Migration 与 Clone 聚焦为 9/9 passed，E05 S01–S04 与受影响 Clone 的真实 SQL 串行链为 11/11 passed；默认门禁仍保留环境隔离 skip 语义，不能把未启动的 SQL-gated 测试写成通过。
