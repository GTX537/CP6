...
- 本机 DEV/UAT/PROD-LAB Docker 运行边界已建立并实际验证。下一人工步骤是在 Azure DevOps `Pipelines → Environments` 以 Resource `None` 创建 `cp6-dev`、`cp6-uat`、`cp6-prod-lab`，并保存外部截图/URL 证据。
- 创建 Release Pipeline 后，把三个 Environment 的 Pipeline permissions 收紧到该 Pipeline；DEV 自动，UAT/PROD-LAB 再配置审批与 exclusive lock。单人学习期 PROD-LAB 可自批，真实生产必须换独立批准人。
...
- 下一切片 Intake：实现人工录入和官网公开提交、线索池查询、负责人分配/移交、协作人、活动时间线、4 自然小时 SLA、重复候选与受控合并；数据范围必须按负责人/协作人/部门/管理员显式校验。
- 后续切片：企业/联系人与商机转化；报价接受和 ERP 订单桥接；CRM Vue 工作台；营销官网 CMS/多语言 SSR 发布；PII 24 个月匿名化、SLA 通知和漏斗/来源报表。
