...
- WP4 详细 Spec LM-FR-017～029 的编辑器系列已有仓库实现；两点实距标定、构件库、统一历史、2D/3D 同源、选择/视角/未保存状态、四步清单、问题筛选定位和窄屏只读均有自动化。LM-FR-004 的后端 SHA-256 复用合同和当前 CAD/底图直接上传提示、LM-FR-005 的来源引用预检/双 Revision Fence/幂等软删除/文件与审计保留、LM-FR-012 的 CAD 图层/块清单、LM-FR-013 的逐层 Override 与租户私有 Mapping Profile、LM-FR-014～016 的七类语义/来源证据/稳定质量问题均已闭环。统一 Excel 上传 UI 仍待三路径向导；三条路径主链还须审计 LM-FR-002 的模板来源后续项、LM-FR-003 的生产等价安全扫描证据、LM-FR-010～011、019/019A 与当前 Typed Changeset/Provider 实现的剩余差距。上述仓库闭环不自动把 WP4 标回 Complete，也不得用 Mock/fixture 替代真实 DWG/DXF/Excel/PDF、Provider、WMS 与 Pilot 接受。
