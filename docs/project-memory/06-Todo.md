...
- 核心 GA 当前派生结果为 `NoGo`：3 类真实外部输入、WP0–WP8 九个结果门禁和 1 个 DeliveryOwner 签署均 Pending。下一步只需按 `kickoff-evidence-protocol.md` 登记 20 CAD 候选、至少两条 Provider 审批链/隔离 Worker、Greenfield/Retrofit 双仓和 WMS 窗口。同一实名 Owner 可以拥有、接受和签署全部内部门禁；模板、fixture、哈希错误、占位身份或不完整技术证据仍不得改状态。
...
- WP7 的正式黄金 CAD Manifest、证据协议、失败关闭专项校验和总 GA 组合门禁已完成；模板、fixture、DevelopmentSeed 和人工改写汇总不能冒充正式证据。仍须用 20 份授权真实黄金 CAD 执行 10/5/5 Calibration/Validation/Holdout，由一个实名复核人完成可追溯复核，并在同一 Source Set/冻结 Worker 上运行主备 Provider，产出 release-eligible 质量/Wilson/人工操作、Holdout Blocking 与 50 MiB/Ready P95 的受控证明。
