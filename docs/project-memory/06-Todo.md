...
- WP4 详细 Spec LM-FR-017 的改类型、删除、合并、拆分、画布重画，LM-FR-018 的持久人工校正锁/重新解析 Blocking Conflict，以及 LM-FR-023 的对齐、等距、复制、旋转、阵列已有仓库实现。下一独立任务先审计并关闭 LM-FR-024 的统一撤销/重做，重点核对 CAD/Excel/底图导入确认批次；随后继续逐项复核 LM-FR-020～029 与三条路径。上述仓库闭环不自动把 WP4 标回 Complete，也不得用 Mock/fixture 替代外部接受。
