...
- AutoCAD 2025 Core Console 的 DWG 候选已通过新的远程隔离 Worker 服务边界：完整源 SHA 核对、统一合同执行、CAD IR-only 响应、Attempt 清除和精确版本绑定均有真实安装测试。它仍不是已认证 Site Provider；下一步须形成非 `development` 的发布身份，确认许可证允许的 Worker/自动化部署边界，证明真实 mTLS、禁网/出口控制、专用身份/临时盘/清除失败关闭，取得客户/Site 批准，并补齐 DXF。GUI `acad.exe` 的 `HashMismatch` 仍应通过 Autodesk 更新/修复或受批准例外处理；候选 Worker 当前只调用签名有效的 Core Console。未完成这些条件前不得填写 `acceptedEvidence` 或关闭 WP3/WP7。
...
- WP3 的 Site 级认证/路由、ADR-0001 资格证据、80 分门槛、共合同执行器，以及远程 Worker 协议、受限 HTTPS 客户端、严格批准 Manifest 和生产注册均已完成仓库基础；AutoCAD DWG 候选 Host 也已可运行。下一张 Provider 任务卡应补一个技术与故障域独立、覆盖 DWG/DXF 的 Backup 候选，并把 Primary 的真实 DXF 路径闭环；两条链都必须只通过 `SpaceCadConverterContractRunner`，不得把供应商类型写入领域层、让客户端指定任意 Provider Key，或把候选 Host 的存在冒充 Site 批准。
