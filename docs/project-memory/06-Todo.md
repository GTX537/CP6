...
- 以 `a3864d9c` 为当前 Space 代码集成基线，`335659b2` 为对应文档基线。E05 S01–S05、E13 S01–S03/S12 均已完成，不要重复实现。
- 下一张可独立推进的 3D Space 卡为 E04 S01：PDF/PNG/JPG 底图上传和渲染。必须复用 E01 文件安全生命周期，让 `underlay` 真正显示，并覆盖显隐、透明度和锁定；不提前混入 S02 标定或 S03/S04 编辑命令。
- E04 后续固定为 S02 两点标定 → S03 通用元素选择/属性面板 → S04 多选/对齐/分布/阵列；S04 完成后才解除 E07 S05 的采用前置条件。
...
- 本机 `KOUSQLSERVER` 的 Windows 集成认证已可用于提权测试。E05 S01–S04 与受影响 Clone 的真实 SQL 串行链为 11/11 passed；默认门禁仍保留环境隔离 skip 语义，不能把未启动的 SQL-gated 测试写成通过。
