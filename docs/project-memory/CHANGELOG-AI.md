...
## 2026-09-12：C03 ERP 请求、读取与可审计恢复

新增真实 ERP 集成入口、共同冻结的六类事件、同事务 Inbox/订单/Outbox、技术重试及三类管理员恢复操作。报价接受绑定实际条款和明细；普通 ERP 修改会撤销旧接受。新增 SQL 唯一约束及前向迁移，订单编号覆盖 CRM 与普通 ERP 并发写入。

[本地七项真实传输结果](../evidence/c03/2026-09-12/transport-attempt-3/README.md)与失败历史已归档，后续恢复功能有单独 SQL/HTTP 证据。开发交付尚待集中审查、本地发布和正常 PR 合并；没有 Actions、生产发布或环境替换。
...
## 2026-09-10：按任务集中审查，减少开发过程中的重复验证

- 根据用户确认，在 [AGENTS.md](../../AGENTS.md#审查与验证节奏强制) 新增长期执行规则：按完整任务或 PR 审查，开发中局部验证，修复后增量复查，按实际影响和风险扩大检查。
- 同步状态、完成项和后续执行记忆，明确非阻塞建议、高风险专项验证及证据复用边界；本次不变更工作流、分支保护、发布或部署配置。
...
## 2026-09-09：C01 服务身份与轮换完成本地验证

- 新增单租户服务注册与 client_credentials 签发、真实目录复查、CP6.Services 固定 claims、严格输入/传输限制和通用 SQL 故障响应。JWKS 成功响应可缓存 60 秒，其余响应不可缓存；修复框架提前处理错误 Content-Type 导致 OAuth 错误/缓存头缺失的路径。签名器固定配置快照并注入可控时钟，覆盖五阶段 RSA 轮换。
- 身份 116/116、真实 OIDC SQL/HTTP 48/48 均通过且零跳过；全后端 3059 通过、0 失败、19 个既有跳过明示。原用户认证和租户到期时间的本地语义兼容。生产者契约和独立质量审查均通过，正常远端交付待闭环；完整 C01 仍需要真实消费者跨仓验收。

## 2026-09-09：C 系列启动，C01 设计待审阅

- 新增 [C01 设计草案](../superpowers/specs/2026-09-09-c01-identity-design.md)，明确增量身份桥方案、服务客户端边界、JWKS 轮换和分切片验收；同步四份台账。状态为设计待审阅，没有实现、运行环境变更或生产验收。
...
- Owner 批准 `3.1.3-cp6.2`，仅更新该依赖与实际输出摘要/版本、工具消费端及必要文档；其余源码/工具链/锁记录、漏洞门禁、Secret/审批均不变。
- 两轮独立构建逐字节一致，1651 项 Windows 全量、429 项上游测试/子测试、十项离线保护及完整本地镜像扫描/七项运行时密码学检查通过；format 和四条 actionlint 通过，PR/main 交付尚待完成。完整证据见安全构建记录，P10 不提前结案。
- 更正此前 Vitest main 验收计数为五条工作流、六个作业；不是新增或删减门禁。

## 2026-09-09：P10 预检与合并证据，前端安全审计前置修复

- Linux preflight `34254585813` 与合并后 Windows 全量均为 1648/1648、零失败/跳过；PR #90 正常合入 `241d2d3efc3da94bec8830289ad185f1725f1385`，未绕过严格分支保护。
- main `client-contract 34293825913` 在 .NET/Web 通过后因 npm 审计失败。既有 Vitest 4.1.9 受到 [GHSA-82fw-gwwq-j7x9](https://github.com/advisories/GHSA-82fw-gwwq-j7x9) 影响；同一锁文件的本地只读审计复现两项 moderate 报告，官方修复版为 4.1.11。
- Owner 批准独立依赖修复并允许适当精简重复验证。本轮不重复未改动的 P10 本地全量，Web 完整验证沿用现有 GitHub CI；安全阈值、真实托管验证、签名、人工审批及生产边界不变。依赖修复的远端验收和 P10 正式候选流程尚待完成。
- Vitest 及七个既有家族包由 4.1.9 升至 4.1.11。npm 10 自动生成锁文件遇到 Arborist 空引用后，以官方旧/新版本元数据逐包核对并最小更新版本、关联范围、下载地址和 SHA-512；依赖图大小不变，其他 460 个锁记录完全不变。安装 dry-run 成功，原审计失败回归为零漏洞；未使用强制安装、忽略 peer 冲突或降低审计阈值。
...
- 同一冻结程序集/安全构建输入：Windows 1644/1644 通过，Linux 1643 通过、1 项归档读取时间顺序失败、0 跳过。单 CPU/断网探针仍有 −13.522 秒/+14.149 秒 UTC 跳变；Owner 所述太平洋时区与系统返回 Eastern/UTC−04 不一致，但显示时区不能解释同次 UTC 比较倒退。
- 保留原断言和所有旧失败。现有 P10 受保护环境仅允许 main；提出独立人工审批只读 Linux 预检作为下一决定，未创建环境/工作流、复制 Secret、合并或 dispatch。P10 仍 No-Go。

## 2026-09-08：P10 原盘冷备通过，保留式端点恢复遇系统拒绝

- 经 owner 批准，在 Docker/WSL 停止后持原盘只读共享锁复制并核对两个 SHA-256；109912784896 bytes 原盘与副本一致，原盘修改时间未变。系统盘和配置的备份也通过，副本只读、ACL 私有，无上传或数据清理。
- 原数据目录联接仍正确，未迁移/覆盖数据盘。第一个临时端点目录改名保留后，启动失败位置转到第二个 Secrets Engine 端点；第二个目录重命名遭 Access denied，未改 ACL 或接管所有权。
- 七容器仍未恢复，新的 Linux 测试未运行。建议用户保存其他工作后尝试 Windows 整机重启作为下一诊断，尚未执行；P10 仍 No-Go，未 PR/合并/dispatch。

## 2026-09-08：P10 完整 WSL2 停止获批执行，Docker 恢复出现独立启动故障

- 后续 owner 确认自己点击恢复出厂选项。只读复核原数据盘大小、修改时间未变且 VHDX 标识可读；没有推断数据完整或丢失。下一步冷备及保留原盘的恢复尚待批准，本任务未再次 reset、删除文件或启动空数据盘。
...
- 同一冻结测试输入重验为 1643 通过、1 失败、零跳过；Messaging 实际包下载前后 UTC 倒退约 9.195 秒。原生 Linux 三十秒采样捕获约 -13.027 秒 / +14.108 秒跳变，Windows 同期采样未见跳变，证实不只影响 .NET。
- 保留重启前 55 项失败和重启后 1 项失败的独立 TRX，新增摘要和复现事实；不把失败数减少当作修复。同步四份台账和安全构建记录，未 PR/合并/再次 dispatch；完整 WSL2 重置需另获授权。
...
- 增加默认关闭的 RS256 OIDC、真实 `Sys_User` 单租户/部门及启用角色权限投影，SQL 原子授权码账本与显式 tenant entitlement。
- 桥启用时绑定现有浏览器 refresh 链；全局退出使用 BFF 后端 ID hint 与短时一次性浏览器 ticket，精确撤销原会话，保留其他会话；关闭时维持既有 Cookie 行为。
- 安全复核新增持久化浏览器会话、原登录认证版本与 2FA epoch；显式退出在 SQL 会话锁下批量撤销，真实重用仍触发既有账户级保护，长链无需逐节点遍历。
- 增加真实 SQL 专用门禁及 PR/main TRX；修复后本地 97 项相关测试、22 项 SQL、前端安全返回/type-check/build 通过。配置、迁移兼容、边界和后续验收见 [实施记录](../crm/CRM-OIDC-FIRST-SLICE.md)，不提前关闭 C01/C02 或生产门禁。
- [PR #85](https://github.com/GTX537/CP6/pull/85) 最终 head `af0912160c76085390d4d8863ba07efb5789d9a5` 经七项 PR 检查后正常合并为 `main@9453c9b60e268d6b6a1e9f07dfdb71c7b438f73a`；五个 main 工作流全部成功，含 client-contract `34192061268` 与 SQL `34192061341`。PR/main 的身份桥 SQL 各 22/22 无跳过，完整 CP6.Tests 的 19 项既有其他模块跳过未计入专用 SQL 验收。
- 配套 CRM #50/#51 已完成实现与交付记录的 PR/main 验证；本地四个服务和六个随机测试库已清理。实际提交、运行及 artifact 摘要见[项目当前状态](PROJECT_STATE.md)，本次没有生产部署。

## 2026-09-08：CRM 首片公开文档有限预登记准备与合入

- 按本次实施及文档同步授权，为已审查源提交 `b92e0a23f55e83ba92b6755b5381ef7d0b1e1e27` 的首片增补登记完整摘要集合；准备分支保留历史 `docs/crm`，不发布新增说明。
- 可信模块从校验器自身目录加载，仅接受完整旧集合或完整首片集合；禁止混合、缺文档、新路径、任意改字和待审仓库伪造规则。原历史 payload、批准摘要、Git/GitHub 证据及商业敏感信息门禁原样保留，秘密扫描覆盖全部发现的 CRM 文档。
- 集合回归覆盖两套完整正向及全部部分组合；集成回归继续覆盖原有审批与内容拒绝情形，并增加缺失文档、伪造规则及新增文档敏感内容检查。测试直接执行当前可信校验器，避免未提交校验变更遗漏于 HEAD 快照测试。
- 四份项目记忆同步标明预登记与后续发布边界。先将准备规则按现有门禁合入受保护 `main`，再由实现分支同步消费；main push 路径过滤新增模块与测试并有配置断言，required context、受保护验证步骤、工作流权限及生产授权均未修改。
- [PR #84](https://github.com/GTX537/CP6/pull/84) 已合并为 `main@80cd15deb05922c63fb8849b6142ba7e98331cf3`，全部 PR/main 检查通过；#85 随后同步该基线并提交已登记的完整 13 文件集合。公开文档摘要与历史审批载荷在本次状态记录中保持不变。

## 2026-09-07：文档分类、导航与维护规则

- 建立文档中心和目录维护规则；31 个根目录文件迁入产品、架构、历史需求、历史 ERP 方案、Space 研究及面试分类，迁移清单保存原始 Git blob。
- 补充缺失的主题导航，更新入站 / 出站链接，修复旧手册、源码及设计引用；区分当前规范、历史状态、教材、导出附件和原始资产。
- 新增本地文件链接检查器及行为回归；迁移附件完整性与原有有效链接保留核对通过。验证方式见 [整理记录](../_inventory/2026-09-07-docs-organization.md)。
- CRM 测试的稀疏检出清单同步增加架构目录，20/20 与 54/54 回归通过；历史 Word 的项目结构相对链接由单页兼容入口承接，原始附件保持不变。
- 本次不修改应用代码、数据库、发布流水线、受摘要约束的批准 / 验收材料或部署环境。
...
- CRM final-propagation PR #40 head `8643a5c8b0b1ab54e480c1490f2639afe6fbcca4` 合并为 `main@116018974dc406ab060eec7ccaf34f9303a7e523`；PR/main runs 33499604406/33500324046 和 SQL artifacts 9797173880/9797446436 及 SHA-256 均已记录，完整 CRM 与真实 SQL 门禁通过。
- P09 S01-S06 全部完成；没有借此注册 CRM runtime/Worker/subscription、连接真实集群或数据库，或授权 Secret、云资源、P10、业务切片与部署。
...
- 公共同步完成当时 P09 仍为 `Published / Consumer Candidate`；该历史阶段随后由上方 S06 冻结闭环接续。当时未连接真实集群、注册 CRM runtime 或授权 Secret、云资源、环境部署和 P10/业务切片，当前边界也未改变。
...
- 新增统一解析器并接入画布渲染、越界碰撞、中心线吸附、模板放置预览和巷道统计；保留旧格式兼容，畸形及未知版本几何失败关闭。
- 新增六层回归覆盖；全量 Web 181 files / 956 tests、严格类型检查和生产构建通过。提交 `fc6b545d` 的隔离 Docker 候选读取真实 CP6DB 后，相关接口全部 200、画布与五工具正常、Vue/TypeError 为 0，未执行保存或其他写操作。
- Konva 6/7 Layer 警告作为独立 P2 性能项保留，不影响本次功能验收结论。
...
- `CP6.CRM` PR #29 固定版本消费并复用 P04/P05 失败关闭；PR run 33243227124 attempt 3 与合并后 main run 33244344319 通过远端恢复、pinned real-SQL 和完整门禁。PR #30 把 locator 冻结为 `Frozen / Consumable`，最终 `main@744ca5d9d06db4470d18a4d8ce3ecfbae42f1d2c` run 33245027773 通过。
- CRM-F3-CONTRACT/C02 业务事件、CRM Worker/运行时订阅、Secret、云资源、迁移和部署仍未实现或授权。
