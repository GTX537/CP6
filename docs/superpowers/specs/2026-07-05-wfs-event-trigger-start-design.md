...
| D7 | `Wf_TriggerFire` 流水三种触发器**统一写**：既是审计台账，也是幂等闸（filtered unique index on IdempotencyKey） | ServiceTask Wf_ServiceJob filtered index 先例 |
...
索引：`(TenantId, FlowKey)`；`(Enabled, TriggerType, NextDueUtc)` 扫描索引。
...
幂等键口径：timer = `"{TriggerId}:{NextDueUtc:O}"`（同一到期时刻只发一次）；event = outbox 事件行 Id；message = 调用方 `Idempotency-Key` 头（缺省 400）。

### §2.3 ConfigJson 分型

- **timer**：`{ "cron": "0 0 25 * *", "varsJson": "{...}" }`——cron 5 段（NCrontab 标准），varsJson 为发起时的初始流程变量（可空）。
- **event**：`{ "eventKey": "WMS|OnShipmentConfirmedAsync", "varsMap": { "orderNo": "$.OutboundNo" } }`——eventKey = `{SourceModule}|{HookName}`；varsMap 用点路径/JSONPath 精简子集把事件负载映射进流程变量（复用 `ServiceVarsHelper` 点路径口径，含其已记档限制）。
...
### §3.2 timer：`WfTriggerWorker`（BackgroundService）

- 照抄 `WfServiceJobScanWorker` 骨架：周期扫 `Enabled && TriggerType==Timer && NextDueUtc <= now`。
- 抢占 = 读行→算新 NextDueUtc（NCrontab `GetNextOccurrence`）→ RowVersion 乐观写回；写回成功者获得本次发火权（多实例安全）；随后 FireAsync（幂等键含旧 NextDueUtc，崩溃重扫不双发）。
- **misfire 口径**：宕机跨过多个到期点 → 只补发**最近一次**（NextDueUtc 直接推到未来下一个），不追历史——例行审批场景积压补发无意义且危险。

### §3.3 event：`WfTriggerBridgeHook`（BridgeHook 家族新成员，D4）

- `IWfTriggerBridgeHook.OnEventAsync(string eventKey, string payloadJson, string? userName)`：查 `Enabled && TriggerType==Event && ConfigJson.eventKey==eventKey` 的触发器（可多条，逐条 FireAsync）。
...
- 响应：201 `{instanceId}` / 200 幂等重放返回既有 `{instanceId}` / 401 key 无效 / 404 触发器不存在或未启用（不区分，防枚举）/ 400 缺 Idempotency-Key 或负载超限（上限 64KB）。
...
| 码 | 场景 | 层 |
|---|---|---|
| **E-WF-022** | 触发器配置无效：cron 解析失败 / eventKey 格式错 / varsMap 点路径非法 / StarterUserId 不存在或停用 | 保存时（服务层） |
...
- **timer**：到期扫描发起、NextDueUtc 前移抢占（并发两 worker 模拟只发一次）、misfire 只补最近一次、cron 边界（月末/闰年）。
- **event**：eventKey 匹配多触发器逐发、varsMap 映射、outbox 失败重试路径（dispatcher fallback 路由）、未匹配零动作。
