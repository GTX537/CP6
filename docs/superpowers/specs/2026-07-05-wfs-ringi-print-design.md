...
3. **传签履历表格**：关卡 / 審査者 / 判定（承認・却下・転送・スキップ）/ 日付 / 意見——只打已发生行（D2）；subFlow 节点行备注列显示子单号（D3）。**跨 spec 对齐**：判定词 スキップ（Skipped）会由版本治理 spec 的强制推进产生——双痕落纸面是有意为之，两份计划的 i18n 判定词表**用同一词条键**（出 plan 时对齐）。
4. **印章式签名栏**：按流程关卡横排「枠」（印鑑枠格式：关卡名+氏名+日付），已决关卡填充、未决留白——日企稟議書的视觉惯例。
   - **会签/并行关卡枠规则（spec 评审补）**：一关卡一枠，多审批人（会签 all/any/veto、并行分支同关卡多 FormTo 行）在**枠内纵列多人**，每人一行「氏名+判定+日付」（判定各标——会签里有人承認有人却下都如实落纸）。vitest 三态数据须含一张会签单。
   - **横排折行规则（spec 评审补）**：长流程十几关卡必爆 A4 宽——枠容器 `display:flex; flex-wrap:wrap`，**以枠为单位折行**且每枠 `break-inside:avoid`（枠不横向压缩、不跨页劈开）。QA 走查含长流程单。
