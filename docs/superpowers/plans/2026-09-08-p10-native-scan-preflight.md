...
Subsequent owner decision: approved the reviewed reproducible cosign security rebuild and started Docker for local diagnostics. The original failed image/run/reports remain unchanged. Implementation, independent binary reproduction and actual full-image scan results are in the [security-build record](2026-09-08-p10-cosign-security-build.md); this later authorization does not retrospectively turn the old failure or new local diagnostics into hosted acceptance.
