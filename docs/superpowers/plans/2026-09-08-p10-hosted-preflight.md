...
- [x] Commit only reviewed task files, push the task branch normally and open a normal PR. The new push workflow waits for owner approval; show the exact run link. Existing PR checks may run concurrently, but no merge occurs until hosted P10 preflight and all required PR checks are green on the exact head.
- [x] Inspect complete hosted output, including the reproduced cosign digest, actual package verification, every test count, and TRX identity. Diagnose any new failure instead of changing permissions or skipping gates.
...
## Hosted execution and prerequisite follow-up — 2026-09-09 UTC

- Owner-approved run `34254585813`, attempt `1`, job `102157048115`, exact source `f4ac824e86e3067a4b82df57a796bdef6f9fe53b`: all steps successful. Both cosign binaries reproduce their reviewed digests; seven real package inputs and seven boundary refusals pass; the full suite is **1648 passed / 0 failed / 0 skipped**.
- Diagnostic artifact `10081995323`, name `p10-preflight-f4ac824e86e3067a4b82df57a796bdef6f9fe53b-34254585813-1`, archive SHA-256 `c13253c97ab78e17130453a30556cbddf94003206f9ec3c3dd52e5f0b7fba1bd`. Downloaded TRX SHA-256 `411c82fd8de834fbf0cbbafc31426091e8009ed274bacbc703882ba6c7aece15`; XML confirms all 1648 executed and passed. This remains diagnostic evidence, not a validation handoff.
- PR #90 passed all eight exact-head checks and merged normally as `241d2d3efc3da94bec8830289ad185f1725f1385`; remote ancestry and identical merge tree were verified. The merged Windows full suite also passed 1648/1648 without skips; TRX SHA-256 `4d8a40f3df6b40446ff5d3014f966fec67476691efde18d1846e204f24b1b581`.
- Main run `34293825913` then failed the existing R2 npm audit on unchanged Vitest / `@vitest/mocker` 4.1.9. The same lock audited clean in the earlier PR; the current database now reports [GHSA-82fw-gwwq-j7x9](https://github.com/advisories/GHSA-82fw-gwwq-j7x9). Read-only local audit reproduces two moderate reports. Owner approved a separate dependency-fix branch and proportionate verification: no duplicate local P10 full run for an unchanged verifier; retain complete hosted Web checks and all authoritative validation, signing, security and approval gates.
- Task 4 remains open until the dependency fix passes normal PR/exact-main delivery and the actual validation/publication/audit chain completes. No new formal validation or candidate acceptance is inferred from preflight or merge success.
- Dependency prerequisite local evidence: Vitest plus seven already-locked family packages patched from 4.1.9 to 4.1.11. npm 10 lock generation hit an Arborist null reference, so old/new official package metadata was compared and only the eight records plus the root version range were updated; all other 460 records are identical. Node 22.22.0 / npm 10.9.4 `npm ci --dry-run --ignore-scripts --no-audit --no-fund` exits 0. The original lock-only audit now exits 0 with zero vulnerabilities. Actual installation, type-check, tests and build remain required hosted evidence.
