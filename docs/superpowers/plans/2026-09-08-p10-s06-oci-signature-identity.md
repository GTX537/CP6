...
- The staged whitespace check exposed an extra EOF blank in OciSignatureProfile.cs. The shell sequence did not stop on that native-command exit status and created commit 2170864b. A same-task follow-up removes only that blank, reruns the checks with explicit exit-code enforcement, and preserves the auditable original commit; no behavior changes.
