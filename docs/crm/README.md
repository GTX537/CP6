...
| [CRM V1 产品需求文档](./CRM-V1-PRD.md) | Sanitized candidate for product approval | 对齐 Frozen SaaS V1，定义前端效果、后端逻辑、状态/权限/失败语义、验收和升级边界；候选 payload SHA-256 为 `5e646cc8e394c74c35f9716216be1d12fa5f4f7210e42d8d52ab9b86f4528a3a` |
...
CRM V1 PRD 当前为脱敏审批候选。三次未合并的预审批尝试因脱敏声明不完整而作废，其审计证据保留在未合并 PR 中，不进入 `main` 提交祖先或公开产品基线。新的唯一 `ProgramOwner` 批准必须绑定当前精确 payload 摘要、候选 commit/blob 和五项产品结论；产品批准也不构成开工、Pilot、UAT 或生产批准。当前 `main` 仍只有 Foundation 的 20 表、状态机、迁移、6 个禁用菜单和 22 个动作；没有 CRM Controller、独立 CRM API、Next.js/React Native 客户端或可用 CRM 路由。
