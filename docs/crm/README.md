...
| [CRM V1 产品需求文档](./CRM-V1-PRD.md) | Corrected Candidate for Product Approval | 对齐 Frozen SaaS V1，定义前端效果、后端逻辑、状态/权限/失败语义、验收和升级边界；脱敏修正版 payload SHA-256 为 `6d6ee4e4a8abff80a7b1f8beed1152e914d072e751adb7f3fa43ba3d7aecbac7` |
...
唯一 `ProgramOwner` 对旧 payload 的批准因合并前脱敏审查而被修正版取代；旧评论和 [append-only 历史记录](./approvals/history/2026-08-26-cp6-crm-v1-prd-program-owner.json) 保留但不批准当前摘要。修正版在获得新的精确摘要批准前不构成开工、Pilot、UAT 或生产批准。当前 `main` 仍只有 Foundation 的 20 表、状态机、迁移、6 个禁用菜单和 22 个动作；没有 CRM Controller、独立 CRM API、Next.js/React Native 客户端或可用 CRM 路由。
