...
唯一 `ProgramOwner` 已批准干净分支中的完整脱敏 payload、候选 commit/blob 和五项产品结论，证据固定在 [append-only v4 历史记录](./approvals/history/2026-08-26-cp6-crm-v1-prd-program-owner-v4.json)。三次未合并的预审批尝试因脱敏声明不完整而作废，其审计证据保留在未合并 PR 中，不进入 `main` 提交祖先或公开产品基线。产品批准不构成开工、Pilot、UAT 或生产批准。当前 `main` 仍只有 Foundation 的 20 表、状态机、迁移、6 个禁用菜单和 22 个动作；没有 CRM Controller、独立 CRM API、Next.js/React Native 客户端或可用 CRM 路由。
