...
| [CRM V1 产品需求文档](./CRM-V1-PRD.md) | Approved product requirements baseline | 对齐 Frozen SaaS V1，定义前端效果、后端逻辑、状态/权限/失败语义、验收和升级边界；批准 payload SHA-256 为 `128bda13277a50fa024c8912676d7ed9e842fd6837b7de11d6055eb8e176fc53` |
...
当前 PRD 已由唯一 `ProgramOwner` 对精确 payload 摘要批准，审批证据绑定 PR #33 的候选 commit/blob，并固定在 [append-only 历史记录](./approvals/history/2026-08-26-cp6-crm-v1-prd-program-owner.json)。该批准只冻结产品需求，不构成开工、Pilot、UAT 或生产批准。当前 `main` 仍只有 Foundation 的 20 表、状态机、迁移、6 个禁用菜单和 22 个动作；没有 CRM Controller、独立 CRM API、Next.js/React Native 客户端或可用 CRM 路由。
