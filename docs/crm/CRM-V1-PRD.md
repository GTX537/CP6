...
| 移动 | 不存在 | React Native 功能等价客户端 | 在 Web GA 后通过独立移动 GA 门禁；精确时间表保留在私有采用 Manifest |
...
V1 Epic 只有在产品冻结、M0、四仓合同、设计伙伴、技术候选、Web GA、移动 GA、Lead Adoption、Full Journey Adoption 和最终采用门禁全部通过后才可关闭；各门禁的数值窗口保留在私有采用 Manifest。
...
- Web GA 后必须独立通过移动 GA；精确推广时间表保留在私有采用 Manifest。移动延期不阻止 Web GA，但阻止 V1 Epic 关闭。
...
- Pilot 参与角色、组织覆盖和 cohort 构成保留在私有 Pilot Acceptance Manifest；公开证据只记录角色类别覆盖结果与 Manifest 摘要。
- 固定任务总量、每人任务量和 Website/Manual、重复候选、跨部门移交、无 Owner、并发冲突、SLA 跨工作时段等场景样本阈值保留在私有 Pilot Acceptance Manifest。
- 无引导完成率、成功任务耗时、预期拒绝/恢复、租户与 PII 隔离以及缺陷严重度退出阈值保留在私有 Pilot Acceptance Manifest；公开关闭证据只记录各门禁是否通过和 Manifest 摘要。

### 14.4 上市与采用

- 顺序：CP6 自用验证、受控设计伙伴 Pilot、Web GA、移动 GA。商业 cohort 的数量、地域、名单和精确推广时间表保留在私有采用 Manifest，本公开 PRD 只约束阶段顺序和不可跳过的证据门禁。
- Lead Adoption：观察窗口、Eligible Lead 分母、Website/Manual 覆盖、旧写入退出、分配/异常队列时效和首次响应阈值保留在私有采用 Manifest；公开关闭证据只记录门禁结果和 Manifest 摘要。
- Full Journey：观察窗口、Conversion/OrderRequest 最小样本和 ERP/报表对账阈值保留在私有采用 Manifest；公开关闭证据只记录门禁结果和 Manifest 摘要。
...
| P5 Mobile/Adoption | React Native、Push、商店分发、采用门禁 | Web GA | 移动 GA、Lead/Full Journey 与最终采用门禁通过；数值窗口见私有采用 Manifest |
