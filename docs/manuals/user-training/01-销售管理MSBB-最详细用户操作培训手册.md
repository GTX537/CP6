...
| 13 | 销售管理→信用单 | 信用单 CreditNote | 退货调应收凭据(RMA回写) | 只读列表页 | 财务/销售 | 检索 | ✅已实现 | 由WMS RMA产生 |
| 14 | 销售管理→未出荷 | 未出荷 Backorder | 跟单台账(未发齐受注) | 列表+跟单操作 | 销售/跟单 | 检索/Close残数/Split分割 | ✅已实现 | 否(跟单) |
| 15 | 销售管理→OTD报表 | OTD 准时交付报表 | 准时交付率KPI | 只读报表页 | 销售管理/管理层 | 检索/分组/CSV | ✅已实现 | 否(KPI) |
| 16 | 销售管理→為替レート | 為替レート FxRate | 外币汇率(JPY基轴/受注冻结) | 主数据CRUD | 财务/价格管理 | CRUD | ✅已实现 | 是(受注冻结FxRate) |
| 17 | 销售管理→取引先一覧 | 取引先一覧 | 取引先检索台账(11FLG/CSV) | 查询列表页 | 主数据/销售 | 检索/CSV/参照訂正 | ✅已实现 | 否(检索) |
| 18 | 销售管理→版型・木型 | 版型・木型 PlateMold | 印版/木型主数据(采番MLD/改訂Rev) | 主数据+一覧 | 工务/版型管理 | 登录/改定/修正/删除/标签 | ✅已实现 | **是(自动建購買受注)** |
| 19 | 销售管理→FSC チェック | FSC チェック | FSC製品检查表发行 | 检索+批量发行 | 环境/品质/销售内勤 | 检索/批量发行 | ✅已实现 | 间接(回写FSC管理No) |
...
### 5.13 信用单 CreditNote（クレジットノート）

#### 5.13.1 页面业务目的
**退货后调整应收的凭据台账**。信用单**不在本页手工建**——它由 **WMS RMA(返品)确定** 通过 Bridge Hook(`ErpBridgeHook.OnReturnConfirmedAsync`)**自动回写产生**：RMA 确定时累加受注明细 `ReturnedQty` 并生成一张 CreditNote。本页是**只读列表**，按客户/受注号/类型/日期检索这些信用单，看类型(REFUND退款/EXCHANGE换货/SCRAP报废)、金额、关联受注/RMA号、事由。

#### 5.13.1a 业务前置检查清单（操作前必看）

| 检查项 | 为什么要检查 | 不满足会怎样 | 处理方法 |
|---|---|---|---|
| 是否已有 RMA 返品确定 | 信用单由RMA产生 | 列表空 | 先在WMS走RMA返品 |
| 知道是只读台账 | 本页不能建/改 | 找不到新建按钮 | 退货走WMS RMA |
| 信用单类型含义 | REFUND/EXCHANGE/SCRAP | 误读 | 见§5.13.10 |

#### 5.13.1b 关键字段业务填写口径

| 字段 | 业务含义 | 谁提供 | 说明 |
|---|---|---|---|
| 信用单NO `creditNoteNo` | 信用单编号 | 系统(RMA回写采番) | 只读 |
| 受注NO `webOrderNo` | 关联受注 | RMA回写 | 可点跳受注 |
| RMA号 `rmaNo` | 关联返品单 | WMS RMA | 来源 |
| 类型 `type` | REFUND/EXCHANGE/SCRAP | RMA定 | 见§5.13.10 |
| 金额 `amount` | 调整应收的金额 | RMA回写 | 财务依据 |
| 事由 `reason` | 退货原因 | RMA | >50字截断+tooltip |

#### 5.13.1c 灰按钮 / 不可操作说明

| 元素 | 说明 |
|---|---|
| (无新建/编辑/删除) | 本页纯只读;信用单由RMA自动产生 |
| 受注NO链接 | 点跳 `/order?webOrderNo=`查受注 |

#### 5.13.1d 完成后检查点与下游验证

| 操作 | 当前页面检查 | 关联检查 | 测试关注点 |
|---|---|---|---|
| RMA后查信用单 | 列表出现对应信用单 | 受注 ReturnedQty 累加;财务应收调整(`待业务确认`AR链路) | RMA→信用单链 |
| 点受注链接 | 跳受注查原单 | 受注退货数 | 跳转 |

#### 5.13.1e 详细操作场景（SOP）

##### 场景一：退货后查信用单并核对应收
- **业务背景**：客户退货，WMS 走完 RMA，财务要核对信用单调应收。
- **使用数据**：客户 C0001、受注 WO...、类型 REFUND。
- **前置条件**：WMS RMA 返品已确定。
- **详细操作步骤**：1) 进【信用单】;2) 填客户/受注号/类型/日期检索;3) 看信用单(金额/类型/事由);4) 点受注NO跳受注核对;5) 财务据此调应收。
- **完成后检查**：信用单金额=退货金额;受注 ReturnedQty 累加。
- **异常处理**：查不到→确认RMA是否确定。
- **测试用例拆分**：RMA→信用单(P1)、金额核对(P1)。

##### 场景二：按类型筛选(退款/换货/报废)
- **业务背景**：分别看不同处理类型。
- **使用数据**：类型=REFUND/EXCHANGE/SCRAP。
- **详细操作步骤**：选类型→检索。
- **完成后检查**：仅该类型。
- **测试用例拆分**：类型过滤(P2)。
...
| 角色 | 使用目的 | 常用操作 | 权限说明 |
|---|---|---|---|
| 财务 | 据信用单调应收 | 检索/核对 | `[Authorize]` |
| 销售/客服 | 查退货处理 | 检索 | 同上 |

#### 5.13.3 进入路径
系统菜单 → 销售管理 → 信用单（`/erp/credit-note`）。

#### 5.13.4 页面区域说明

| 区域 | 作用 | 关注 | 备注 |
|---|---|---|---|
| 检索区 | 客户/受注号/类型/日期 | 类型筛选 | — |
| 列表 | 信用单台账(只读) | 金额/类型 | 受注号可跳 |
| 分页 | 翻页 | — | 50/100/200 |
...
| 受注NO `webOrderNo` | 受注过滤 | 否 | — |
| 类型 `type` | ALL/REFUND/EXCHANGE/SCRAP | 否 | — |
| 日期 from/to | 发行日范围 | 否 | — |
...
| 字段 | 含义 |
|---|---|
| issueDate 发行日 / creditNoteNo 信用单NO / webOrderNo 受注(可跳) / rmaNo RMA号 / customer 客户 / type 类型(tag) / productCd 产品 / qty 数量 / amount 金额 / reason 事由 | 见 5.13.1b |

#### 5.13.7 新增/编辑表单字段说明
**本页无新增/编辑**(信用单由 WMS RMA 自动回写产生)。
...
#### 5.13.10 状态流转说明（信用单类型）

| 类型 | 业务含义 | 着色 | 是否RMA自动生成 |
|---|---|---|---|
| REFUND 退款 | 退货退款 | warning | **是(RMA回写硬编码)** |
| EXCHANGE 换货 | 换货 | info | 否(枚举预留,无生成逻辑) |
| SCRAP 报废 | 退货报废 | danger | 否(枚举预留,无生成逻辑) |

> 实际产生恒为 REFUND;EXCHANGE/SCRAP 无自动生成(代码未发现)。金额 amount 当前 API 返回 null。**AR 调整=财务手工**在 /fin/ar/invoice 建红字(指定 CreditNoteId)→收入冲销+成本回冲双凭证→AR 负余额(已实证)。

#### 5.13.11 标准操作步骤
**退货核对**：WMS RMA确定→本页查信用单→核对金额/受注ReturnedQty→财务调应收。测试点：RMA→信用单链。

#### 5.13.12 常见问题与处理方法

| 问题 | 原因 | 处理 | 测试关注点 |
|---|---|---|---|
| 查不到信用单 | RMA未确定/条件严 | 确认RMA;放宽条件 | RMA前置 |
| 想新建信用单 | 本页只读 | 退货走WMS RMA | 只读 |
| 金额对不上 | 退货数量/单价 | 核对RMA与受注 | 金额 |

#### 5.13.13 用户注意事项
- 信用单**由 WMS RMA 自动产生**,本页只读;退货走 WMS RMA 流程(手册06 WMS)。
- 信用单调整应收=**财务手工**在【应收发票 /fin/ar/invoice】建红字(指定 CreditNoteId)→自动生成"收入冲销+成本回冲"双凭证、AR 余额转负(已实证,非自动)。
- 与受注关系:RMA 回写累加受注明细 `ReturnedQty`(见手册10 闭环 OnReturnConfirmedAsync)。

#### 5.13.14 本页面测试点汇总

| 编号 | 测试类型 | 测试内容 | 预期 | 优先级 |
|---|---|---|---|---|
| CN-01 | 链路 | RMA确定产生信用单 | 列表出现 | P1 |
| CN-02 | 查询 | 按客户/受注/类型/日期 | 命中 | P1 |
| CN-03 | 跳转 | 点受注NO | 跳受注 | P2 |
| CN-04 | 类型 | REFUND/EXCHANGE/SCRAP过滤 | 仅该类型 | P2 |
| CN-05 | 只读 | 找新建/编辑 | 无(只读) | P2 |
| CN-06 | 金额 | 信用单金额=退货金额 | 一致 | P1 |

> 单页 SOP 详见 `sales-msbb-pages/01-S05-信用单CreditNote-单页面操作SOP.md`。

---

### 5.14 未出荷 Backorder（バックオーダー）

#### 5.14.1 页面业务目的
**销售跟单核心台账**——列出"没发齐"的受注明细(出荷状态未完了、残数>0)，让销售盯着尾单。每行显示 受注数/出荷済数/残数(remainingQty)/最終出荷日；并提供两个跟单动作：**残数クローズ(Close)**(放弃剩余、关掉这条未出荷)与**分割(Split)**(把残数拆成一张新受注继续跟)。

#### 5.14.1a 业务前置检查清单（操作前必看）

| 检查项 | 为什么要检查 | 不满足会怎样 | 处理方法 |
|---|---|---|---|
| 有未发齐的受注 | 台账才有内容 | 列表空 | 先有部分出荷的受注 |
| Close/Split 要填理由 | 审计/跟单 | 理由空被拦 | 填理由(≤100字) |
| 懂 Close vs Split 区别 | 一个放弃一个续跟 | 误操作 | 见§5.14.10 |

#### 5.14.1b 关键字段业务填写口径

| 字段 | 业务含义 | 谁提供 | 说明 |
|---|---|---|---|
| 受注NO `webOrderNo` | 受注 | 系统 | 可跳受注 |
| 受注数/出荷済数 `orderedQty/shippedQty` | 订/已发 | 系统 | 对照 |
| 残数 `remainingQty` | 未发齐数量 | 系统 | 橙色强调,跟单核心 |
| 最終出荷日 `lastShipDate` | 上次出荷日 | 系统 | 跟单参考 |
| 理由 `reason` | Close/Split 原因 | 用户 | 必填≤100字 |

#### 5.14.1c 灰按钮 / 不可操作说明

| 按钮 | 何时不可用 | 为什么 | 处理 | 测试关注点 |
|---|---|---|---|---|
| Close/Split 确认 | 理由空 | 必填 | 填理由 | 必填 |

#### 5.14.1d 完成后检查点与下游验证

| 操作 | 当前页面检查 | 关联检查 | 测试关注点 |
|---|---|---|---|
| Close 残数 | 该行消失/残数清 | 受注出荷状态完了(`待业务确认`机制) | 关闭 |
| Split 分割 | 提示新受注号 | 新受注承接残数(`待业务确认`字段) | 分割 |
...
- **使用数据**：客户 C0001、日期范围。
- **前置条件**：有未发齐受注。
- **详细操作步骤**：1) 进【未出荷】;2) 填客户/日期检索;3) 看残数列;4) 点受注号跳受注核对。
- **完成后检查**：列出残数>0 的明细。
- **测试用例拆分**：未出荷查询(P1)。
...
- **业务背景**：剩余不再发，关掉。
- **使用数据**：某残数行。
- **详细操作步骤**：1) 点【Close】;2) 弹框填理由;3) 确认→关闭该未出荷。
- **完成后检查**：该行消失;受注尾单关闭。
- **异常处理**：理由空→拦截。
- **测试用例拆分**：Close+理由必填(P1)。

##### 场景三：分割残数为新受注
- **业务背景**：剩余改期续发,拆新单跟。
- **使用数据**：某残数行。
- **详细操作步骤**：1) 点【Split】;2) 填理由;3) 确认→生成新受注号(提示)。
- **完成后检查**：提示新受注号;新单承接残数。
- **异常处理**：理由空→拦截。
- **测试用例拆分**：Split生成新单(P1)。
...
| 角色 | 使用目的 | 常用操作 | 权限说明 |
|---|---|---|---|
| 销售/跟单 | 盯尾单 | 检索/Close/Split | `[Authorize]` |

#### 5.14.3 进入路径
系统菜单 → 销售管理 → 未出荷（`/erp/backorder`）。
...
|---|---|---|---|
| 检索区 | 客户/日期范围 | — | — |
| 列表 | 未出荷明细(残数) | 残数 | 受注号可跳 |
| 行操作 | Close/Split | 各需理由 | — |
| 弹框 | 填理由确认 | 理由必填 | ≤100字 |
...
#### 5.14.6 列表字段说明
webOrderNo(跳)/客户/detailNo/productCd/orderedQty/shippedQty/backorderQty/remainingQty(残数)/lastShipDate;见 5.14.1b。

#### 5.14.7 新增/编辑表单字段说明
无新增表单;Close/Split 弹框仅填理由。
...
| 按钮 | 功能 | 前置 | 点击后 | 测试关注点 |
|---|---|---|---|---|
| 検索/リセット/刷新 | 查询 | — | queue | 检索 |
| 受注NO链接 | 跳受注 | — | /order | 跳转 |
| Close 残数クローズ | 关闭残数 | 理由 | closeRemaining | 关闭 |
| Split 分割 | 残数拆新单 | 理由 | splitToNewOrder→新号 | 分割 |

#### 5.14.10 状态流转说明

| 动作 | 含义 | 对受注影响(实证) |
|---|---|---|
| Close | 放弃残数 | 明细 ShipStatus=9 + backorderQty=remainingQty |
| Split | 残数转新单 | 生成新受注承接残数(newWebOrderNo) |

#### 5.14.11 标准操作步骤
**跟单**：检索未出荷→看残数→Close(放弃)或Split(续跟)。测试点：Close/Split。
...
| 列表空 | 无未发齐/条件严 | 放宽 | 空 |
| 理由必填 | 没填 | 补理由 | 必填 |
| Close/Split 区别 | 概念 | Close放弃/Split续跟 | 概念 |

#### 5.14.13 用户注意事项
- 残数 remainingQty = 受注数 − 出荷済数 − Backorder数;残数>0 且受注开放状态(Confirmed/InProduction/PartiallyCancelled)才入台账。
- Close=放弃剩余:该明细置 **ShipStatus=9 + backorderQty=remainingQty**;Split=生成新受注承接残数(返回新号)。
- 非开放状态/已关闭/无残数明细禁操作(后端拦)。

#### 5.14.14 本页面测试点汇总

| 编号 | 测试类型 | 内容 | 预期 | 优先级 |
|---|---|---|---|---|
| BO-01 | 查询 | 未出荷列表 | 残数>0明细 | P1 |
| BO-02 | 关闭 | Close残数 | 该行关闭+理由 | P1 |
| BO-03 | 校验 | Close理由空 | 拦截 | P1 |
| BO-04 | 分割 | Split | 生成新受注号 | P1 |
| BO-05 | 跳转 | 受注号 | 跳受注 | P2 |

> 单页 SOP 详见 `sales-msbb-pages/01-S06-未出荷Backorder-单页面操作SOP.md`。

---

### 5.15 OTD 准时交付报表（On-Time Delivery）

#### 5.15.1 页面业务目的
**销售闭环验收的 KPI 报表**——统计"准时交付率(OTD)"：在选定期间内，已出荷的受注里有多少**实际出荷日 ≤ 客先納期**(准时)、多少超期，给出准时率、超期数、平均超期天数；可按**客户**或**月份**分组、出 CSV。纯只读报表(KPI卡+条形图+表格)。

#### 5.15.1a 业务前置检查清单（操作前必看）

| 检查项 | 为什么要检查 | 不满足会怎样 | 处理方法 |
|---|---|---|---|
| 有已出荷受注 | 才有OTD样本 | 报表空 | 先有出荷实绩 |
| 受注有客先納期+受注日 | 计算准时需納期 | 该单不计入 | 受注录納期 |
| 日期范围合理(按受注日) | 聚合范围 | 漏/多 | 调范围 |

#### 5.15.1b 关键字段业务填写口径

| 字段 | 业务含义 | 说明 |
|---|---|---|
| 准时率 onTimeRate | 准时订单/已出荷订单 | 0~1,显百分比 |
| 超期天数 lateDays | 実出荷日−客先納期 | ≤0 准时 |
| 平均超期 avgLateDays | 仅超期单平均 | 2位小数 |
| 分组 groupBy | customer/month | 客户/月份 |

#### 5.15.1c 灰按钮 / 不可操作说明
纯只读报表,无灰按钮场景;导出CSV受浏览器下载策略。

#### 5.15.1d 完成后检查点与下游验证

| 操作 | 当前页面检查 | 关联检查 | 测试关注点 |
|---|---|---|---|
| 查OTD | KPI(准时率/超期)+分组 | 与受注出荷实绩一致 | 口径 |
| 导出 | CSV含行+KPI | — | 导出 |
...
##### 场景一：看本季准时交付率
- **业务背景**：管理层要看交付 KPI。
- **使用数据**：日期范围(默认近90天)、分组=客户。
- **前置条件**：有出荷实绩。
- **详细操作步骤**：1) 进【OTD报表】;2) 选日期/分组;3)【検索】;4) 看准时率卡(绿)/超期数(红)+条形图+表格。
- **完成后检查**：准时率=准时数/已出荷数;超期单 lateDays>0。
- **测试用例拆分**：OTD计算(P1)。

##### 场景二：按月份看趋势 + 导出
- **业务背景**：看月度趋势给报告。
- **使用数据**：分组=月份。
- **详细操作步骤**：1) 分组=月份→検索;2) 看各月准时率;3)【CSV导出】。
- **完成后检查**：各月yyyy-MM行;CSV含KPI。
- **测试用例拆分**：分组(P2)、导出(P1)。
...
| 角色 | 使用目的 | 常用操作 | 权限说明 |
|---|---|---|---|
| 销售管理/管理层 | 看交付KPI | 检索/导出 | `[Authorize]` |
| 销售 | 看自己客户准时率 | 检索 | 同上 |

#### 5.15.3 进入路径
系统菜单 → 销售管理 → OTD报表（`/erp/otd-report`）。

#### 5.15.4 页面区域说明

| 区域 | 作用 | 关注 | 备注 |
|---|---|---|---|
| 检索区 | 日期/分组/客户+导出CSV | 日期默认近90天 | — |
| KPI卡 | 准时率/总出荷/超期 | 准时率绿/超期红 | — |
| 条形图 | Top12分组准时率 | 色梯度好绿/警黄/差红 | 降序 |
| 表格 | 全分组(进度条) | 准时率 | — |
...
| 栏位 | 含义 | 必填 | 默认 |
|---|---|---|---|
| 日期 from/to | 按受注日聚合 | 否 | 近90天 |
| 分组 groupBy | customer/month | — | customer |
| 客户 customerCd | 过滤 | 否 | — |

#### 5.15.6 列表字段说明（分组行）
groupKey/groupLabel/totalShippedOrders(已出荷数)/onTimeCount(准时)/lateCount(超期)/onTimeRate(准时率)/avgLateDays(平均超期)。
...
#### 5.15.10 状态流转说明
报表无状态机;核心是**准时判定口径**:筛选 ShipStatus≥5(已出荷)+有客先納期+有受注日;lateDays=actualShipDate−客先納期(actualShipDate取Order.ActualShipDate否则明细最大LastShipDate);≤0准时。

#### 5.15.11 标准操作步骤
**看KPI**：选期间/分组→検索→看准时率/超期→导出。测试点：OTD口径。

#### 5.15.12 常见问题与处理方法

| 问题 | 原因 | 处理 | 测试关注点 |
|---|---|---|---|
| 报表空 | 无出荷/范围无数据 | 放宽范围 | 空 |
| 某单不计入 | 无客先納期/未出荷 | 受注补納期 | 口径 |
| 准时率异常 | 出荷日/納期口径 | 核对actualShipDate与納期 | 口径 |

#### 5.15.13 用户注意事项
- 准时=実出荷日≤客先納期;超期=実出荷日>納期。
- 仅 ShipStatus≥5(已出荷)且有納期/受注日的受注计入。
- 日期范围按**受注日**聚合(非出荷日)。
...
| 编号 | 测试类型 | 内容 | 预期 | 优先级 |
|---|---|---|---|---|
| OTD-01 | 计算 | 准时率 | =准时/已出荷 | P1 |
| OTD-02 | 分组 | 客户/月份 | 正确分组 | P2 |
| OTD-03 | 超期 | lateDays>0计超期 | 超期数对 | P1 |
| OTD-04 | 导出 | CSV | 含行+KPI | P1 |
| OTD-05 | 口径 | 仅已出荷+有納期 | 过滤正确 | P1 |

> 单页 SOP 详见 `sales-msbb-pages/01-S07-OTD准时交付报表-单页面操作SOP.md`。

---

### 5.16 為替レート FxRate（汇率）

#### 5.16.1 页面业务目的
**外币订单的汇率主数据**。基轴通货固定 **JPY**；为各外币(USD/EUR…)按**生效日(rateDate)**维护汇率(外币1单位=Rate JPY，6位小数)。受注录入时按"客户通货+受注日"取**当日最新汇率**冻结进 `Order.CurrencyCd/FxRate`。本页是**完整 CRUD** 主数据维护(列表+弹框)。

#### 5.16.1a 业务前置检查清单（操作前必看）

| 检查项 | 为什么要检查 | 不满足会怎样 | 处理方法 |
|---|---|---|---|
| 通货CD(ISO 4217 3字) | 唯一标识币种 | 必填E/PA-MSG-FX-001 | 填3字币种 |
| 不是基轴JPY | JPY无需汇率 | 拒PA-MSG-FX-002 | 仅外币 |
| 汇率>0 | 有效汇率 | 拒PA-MSG-FX-003 | 填正数 |
| 生效日(rateDate) | 算"当日最新"用 | 缺则取不到 | 填生效日 |

#### 5.16.1b 关键字段业务填写口径

| 字段 | 业务含义 | 谁提供 | 说明 |
|---|---|---|---|
| 通货CD `currencyCd` | 币种(ISO4217 3字,自动大写) | 财务 | 非JPY |
| 生效日 `rateDate` | 汇率生效日 | 财务 | 默认今天 |
| 汇率 `rate` | 外币1=Rate JPY(6位小数) | 财务 | >0 |
| 备注 `remarks` | ≤200字 | — | 可选 |
...
| 元素 | 何时不可用 | 为什么 |
|---|---|---|
| 为JPY建汇率 | 任何时候 | 基轴无需(PA-MSG-FX-002) |
| 保存 | 通货空/汇率≤0 | 校验拦 |
...
| 操作 | 当前页面检查 | 关联检查 | 测试关注点 |
|---|---|---|---|
| 建/改汇率 | 列表反映 | 受注录入按客户通货+受注日取当日最新冻结 | 冻结 |
| 删除 | 软删消失 | 受注已冻结的FxRate不变 | 软删 |

#### 5.16.1e 详细操作场景（SOP）

##### 场景一：维护一个外币汇率
- **业务背景**：USD 汇率更新。
- **使用数据**：通货 USD、生效日今天、汇率 150.000000。
- **前置条件**：有价格管理权限。
- **详细操作步骤**：1) 进【為替レート】;2) 新建→填通货USD/生效日/汇率;3) 保存。
- **完成后检查**：列表出现 USD/日期/150;受注 USD 单按当日最新取。
- **异常处理**：填JPY→FX-002;汇率0→FX-003。
- **测试用例拆分**：建汇率(P1)、JPY拒(P1)。

##### 场景二：受注录入冻结汇率(下游)
- **业务背景**：建外币受注验证冻结。
- **使用数据**：客户(通货USD)。
- **详细操作步骤**：1) 受注入力选该客户;2) 系统按客户通货+受注日 ResolveForCustomer 取当日最新汇率;3) 冻结进 Order.CurrencyCd/FxRate。
- **完成后检查**：受注 FxRate=当日最新;JPY客户FxRate=1.0;缺率→PA-MSG-FX-404禁保存。
- **测试用例拆分**：冻结(P1)、缺率拒(P1)。
...
| 角色 | 使用目的 | 常用操作 | 权限说明 |
|---|---|---|---|
| 财务/价格管理 | 维护汇率 | CRUD | `[Authorize]` |
| 销售(外币单) | 看现行汇率 | 查看 | 同上 |

#### 5.16.3 进入路径
系统菜单 → 销售管理 → 為替レート（`/erp/fx-rate`）。

#### 5.16.4 页面区域说明

| 区域 | 作用 | 关注 | 备注 |
|---|---|---|---|
| 检索/工具 | 通货CD过滤+刷新+新建 | 基轴JPY标签 | — |
| 列表 | currencyCd/rateDate/rate(6位)/remarks+编辑·删除 | 现行汇率 | 按通货ASC/日期DESC |
| 弹框 | 新建/编辑(通货/日期/汇率/备注) | 校验 | — |
...
#### 5.16.6 列表字段说明
currencyCd/rateDate(日期)/rate(6位小数)/remarks + 编辑/删除按钮。

#### 5.16.7 新增/编辑表单字段说明

| 字段 | 必填 | 校验 |
|---|---|---|
| currencyCd(3字大写) | 是 | 非JPY(FX-002) |
| rateDate(默认今天) | 是 | — |
| rate(min0 precision6 step0.5) | 是 | >0(FX-003) |
| remarks(≤200) | 否 | — |
...
#### 5.16.10 状态流转说明
无业务状态机;关键是 **生效日**决定受注取"当日最新汇率";基轴 JPY→恒 1.0;软删 IsDeleted。

#### 5.16.11 标准操作步骤
**维护汇率**：新建外币汇率(通货+生效日+汇率)→受注按客户通货+受注日取当日最新冻结。测试点:CRUD+冻结。

#### 5.16.12 常见问题与处理方法

| 问题 | 错误码 | 处理 | 测试关注点 |
|---|---|---|---|
| 通货必填 | PA-MSG-FX-001 | 填通货 | 必填 |
| 给JPY建汇率 | PA-MSG-FX-002 | 仅外币 | 基轴 |
| 汇率≤0 | PA-MSG-FX-003 | 填正数 | 正数 |
| 受注缺率 | PA-MSG-FX-404 | 先建该日汇率 | 冻结 |
| 删不存在 | PA-MSG-FX-NOTFOUND | 核对 | — |

#### 5.16.13 用户注意事项
- 基轴 JPY(FxConstants.BaseCurrency);JPY 订单 FxRate 恒 1.0、不建汇率。
- 受注录入时**冻结**汇率(客户通货+受注日的当日最新);缺率禁保存(FX-404)。
- 改/删汇率**不影响已冻结**的受注 FxRate。

#### 5.16.14 本页面测试点汇总

| 编号 | 测试类型 | 内容 | 预期 | 优先级 |
|---|---|---|---|---|
| FX-01 | CRUD | 建外币汇率 | 列表出现 | P1 |
| FX-02 | 校验 | 给JPY建 | FX-002拒 | P1 |
| FX-03 | 校验 | 汇率0 | FX-003拒 | P1 |
| FX-04 | 冻结 | 受注取当日最新 | FxRate冻结 | P1 |
| FX-05 | 冻结 | JPY单 | FxRate=1.0 | P1 |
| FX-06 | 冻结 | 缺率 | FX-404禁保存 | P1 |
| FX-07 | 软删 | 删汇率 | IsDeleted | P2 |

> 单页 SOP 详见 `sales-msbb-pages/01-S04-為替レート-单页面操作SOP.md`。
...
#### 5.17.1 页面业务目的
**取引先マスタ的检索台账**(配 §5.11)。按 11 个属性 FLG(得意先/売掛/請求/入金/納品/与信/発注/買掛/支払予定/支払/メーカ)+ 状态(事前登録/本登録)+ 文本(CD/名/法人番号/住所/担当…)+ 取引先分類 bpClass01~10 检索取引先；可 CSV 全量导出；行操作"参照/訂正"跳到取引先マスタ编辑页。**检索时"属性FLG / 状态 至少各选一"**(E10030 前端拦)。

#### 5.17.1a 业务前置检查清单（操作前必看）

| 检查项 | 为什么要检查 | 不满足会怎样 | 处理方法 |
|---|---|---|---|
| 至少选1属性FLG | FLG做OR过滤 | E10030前端拦 | 勾≥1属性 |
| 至少选1状态 | 事前/本登録筛 | E10030拦 | 勾≥1状态 |
| 检索范围合理 | 件数上限 | E10013 | 缩范围 |
...
| 属性FLG(11) | 角色筛(OR) | 至少1 |
| 状态 | 事前登録/本登録 | 至少1 |
| CD/名/法人番号/住所/担当 | 文本检索 | 模糊/精确 |
| bpClass01~10 | 取引先分類 | 折叠详细区 |

#### 5.17.1c 灰按钮 / 不可操作说明

| 元素 | 何时不可用 | 为什么 |
|---|---|---|
| 検索 | FLG或状态未选 | E10030 至少各选一 |
...
|---|---|---|---|
| 检索 | 结果符合FLG/状态/条件 | — | 过滤 |
| 参照/訂正 | 跳取引先マスタ对应模式 | §5.11编辑页 | 跳转 |
| CSV | 全量导出 | FLG显○/空 | 导出 |

#### 5.17.1e 详细操作场景（SOP）

##### 场景一：找所有"客户(得意先)"本登録取引先
- **业务背景**：导出客户清单。
- **使用数据**：勾【得意先FLG】+【本登録】。
- **前置条件**：有取引先。
- **详细操作步骤**：1) 勾属性FLG=得意先;2) 勾状态=本登録;3)【検索】;4) 看结果/FLG图标;5) CSV导出。
- **完成后检查**：均得意先+本登録;CSV含全量。
- **异常处理**：FLG/状态没勾→E10030;过多→E10013。
- **测试用例拆分**：FLG筛(P1)、E10030(P0)、CSV(P1)。

##### 场景二：打开取引先编辑(参照/訂正)
- **业务背景**：从列表改某客户。
- **使用数据**：某取引先。
- **详细操作步骤**：1) 检索定位行;2) 点【訂正】→跳 `/business-partner?bpCd=&mode=edit`;3) 在取引先マスタ改(§5.11)。
- **完成后检查**：跳对应取引先编辑。
- **测试用例拆分**：跳转(P1)。
...
| 角色 | 使用目的 | 常用操作 | 权限说明 |
|---|---|---|---|
| 主数据/销售 | 检索取引先 | 检索/参照/訂正/CSV | `[Authorize]` |

#### 5.17.3 进入路径
系统菜单 → 销售管理 → 取引先一覧（`/business-partner-list`）。

#### 5.17.4 页面区域说明

| 区域 | 作用 | 关注 | 备注 |
|---|---|---|---|
| 属性FLG筛 | 11 FLG(OR) | 至少1 | — |
| 状态筛 | 事前/本登録 | 至少1 | — |
| 文本/分類筛 | CD/名/担当/bpClass01~10 | 折叠详细 | — |
| 列表 | 取引先(11 FLG图标列) | status色 | 参照/訂正 |
| CSV | 全量导出 | — | UTF-8 BOM |
...
| 栏位 | 含义 | 必填 |
|---|---|---|
| 属性FLG(11个include*) | 角色OR筛 | **至少1(E10030)** |
| 状态(事前/本登録) | 状态筛 | **至少1(E10030)** |
| 登録日 from/to、bpCd、bpName、ein、住所、担当 | 文本 | 否 |
| bpClass01~10 | 分類 | 否 |

#### 5.17.6 列表字段说明
rowNo/status(0事前/1本登録/9削除,着色)/bpCd/bpName/bpAbbrev/salesStaffCd/businessStaffCd/ein/stdCoCd/addr1~2/11 FLG图标(○/-)/createDate/creator。

#### 5.17.7 新增/编辑表单字段说明
本页为检索;建/改在取引先マスタ(§5.11)。
...
| 按钮 | 功能 | 前置 | 测试关注点 |
|---|---|---|---|
| 検索 | 按条件查 | FLG+状态各≥1 | E10030 |
| CSV出力 | 全量导出 | — | 导出 |
| 参照/訂正(行) | 跳取引先マスタ | — | 跳转 |
| 排序 | 白名单(default bpCd) | — | 排序 |

#### 5.17.10 状态流转说明
列表只读检索;status 0事前登録/1本登録/9削除(同§5.11)。

#### 5.17.11 标准操作步骤
**客户检索**：勾属性FLG+状态→文本条件→検索→参照/訂正或CSV。测试点：FLG/状态至少各选一。

#### 5.17.12 常见问题与处理方法

| 问题 | 原因 | 处理 | 测试关注点 |
|---|---|---|---|
| E10030 | FLG或状态未选 | 各勾≥1 | 至少各选一 |
| E10013 件数上限 | 结果过多 | 缩条件 | 截断 |
| 列表空 | 条件无匹配 | 放宽 | 空 |

#### 5.17.13 用户注意事项
- **检索强制"属性FLG/状态至少各选一"**(E10030)。
- 参照/訂正跳取引先マスタ(§5.11);建/改/删在那页。
- CSV 为全量(按检索条件),FLG 显 ○/空。

#### 5.17.14 本页面测试点汇总

| 编号 | 测试类型 | 内容 | 预期 | 优先级 |
|---|---|---|---|---|
| BPL-01 | 校验 | FLG/状态未选 | E10030 | P0 |
| BPL-02 | 检索 | 属性FLG筛 | OR命中 | P1 |
| BPL-03 | 检索 | 状态筛 | 事前/本登録 | P1 |
| BPL-04 | 跳转 | 参照/訂正 | 跳取引先マスタ | P1 |
| BPL-05 | 导出 | CSV全量 | 含FLG○/空 | P1 |
| BPL-06 | 边界 | 件数上限 | E10013 | P2 |

> 单页 SOP 详见 `sales-msbb-pages/01-S02-取引先一覧-单页面操作SOP.md`。

---

### 5.18 版型・木型 PlateMold（ERP 印版/木型主数据）

#### 5.18.1 页面业务目的
**印版/木型(版型・木型)主数据**：维护トムソン抜き型/印刷版等的版型No/名称/種別/改訂、代表製品、打数(使用次数)/限界打数、置場/棚位、到着·出庫·返却日程、供应商等。含**编辑页**(`/plate-mold` PA140)与**一覧检索台账**(`/plate-mold-list` PA160)。**注意区分**：本页是 ERP 版型主数据；WMS 侧 `/wms/plate-mold-stock` 是**版型在庫**(库存/置场)，二者不同(详见手册06 WMS)。一覧支持 ラベル発行(标签)与 picker 模式(从他页选版型带回)。

#### 5.18.1a 业务前置检查清单（操作前必看）

| 检查项 | 为什么要检查 | 不满足会怎样 | 处理方法 |
|---|---|---|---|
| 区分 ERP 版型 vs WMS 版型在庫 | 两功能不同 | 找错页 | ERP主数据/WMS库存 |
| 区间 FROM≤TO | 日期/打数区间 | E10036 | 调正 |
| 版型No/種別等已知 | 检索定位 | 查不到 | 用代表製品/客户辅助 |
| Label発行先勾行 | 标签目标 | E10010 | 勾≥1行 |

#### 5.18.1b 关键字段业务填写口径

| 字段 | 业务含义 | 说明 |
|---|---|---|
| 版型No `wdPtnNo` + 改訂 `wdRev` | 版型编号+改訂 | 主键;onlyLatestRev看最新 |
| 名称 `vrsnName`/種別 `typeClass` | 版名/类别 | — |
| 代表製品 `representativeProductCd` | 关联代表产品 | 受注工程引用版No |
| 打数 `wdQty`/限界打数 `limitWdQty` | 已用/上限次数 | 寿命管理 |
| 最終使用 `achieveDate` | 上次使用日 | — |
| 置場/棚位 `placeCd/shelfLineCd` | 存放位置 | — |
| 到着/出庫予定/返却予定/返却日 | 版型流转日程 | — |
| 刃渡/付数/個数/色数/各型(落丁·置き版·ハンマー·見落とし) | 纸器版型特化 | — |

#### 5.18.1c 灰按钮 / 不可操作说明

| 按钮 | 何时不可用 | 为什么 |
|---|---|---|
| Label発行 | 未勾行 | 无目标(E10010) |
| 選択返却(picker) | picker模式未选行 | E10009 |
...
##### 场景一：检索版型并看明细
- **业务背景**：找某客户某产品的版型。
- **使用数据**：客户/代表製品/onlyLatestRev。
- **详细操作步骤**：1) 进【版型・木型一覧】;2) 填条件(可展开詳細);3)【表示】;4) 点【詳細】跳编辑页(view)。
- **完成后检查**：结果符合;詳細载入版型。
- **异常处理**：区间倒置E10036;0件E10008。
- **测试用例拆分**：检索(P2)、詳細(P2)。

##### 场景二：发行版型标签
- **业务背景**：给版型贴标签。
- **使用数据**：勾若干行。
- **详细操作步骤**：1) 检索;2) 勾発行行;3)【ラベル発行(N)】下载标签CSV。
- **完成后检查**：标签含勾选版型。
- **异常处理**：未勾→E10010。
- **测试用例拆分**：标签(P2)。

##### 场景三：picker 选版型带回
- **业务背景**：他页(PA140)要选一个版型。
- **使用数据**：?picker=1。
- **详细操作步骤**：1) picker模式打开;2) 选行;3)【選択返却】带 wdPtnNo/rev 回。
- **完成后检查**：返回选中版型。
- **异常处理**：未选→E10009。
- **测试用例拆分**：picker(P3)。
...
| 角色 | 使用目的 | 常用操作 | 权限说明 |
|---|---|---|---|
| 工务/版型管理 | 维护/查版型 | 检索/詳細/标签 | `[Authorize]` |
| 销售 | 查产品版型 | 检索 | 同上 |

#### 5.18.3 进入路径
系统菜单 → 销售管理 → 版型・木型(`/plate-mold` 编辑) / 版型・木型一覧(`/plate-mold-list`)。

#### 5.18.4 页面区域说明

| 区域 | 作用 | 关注 | 备注 |
|---|---|---|---|
| 基础检索 | 拠点/手配/案件/版型No区间/種別/客户/代表製品/工程/供应商/置場 | 多条件 | — |
| 詳細检索(折叠) | 打数/限界/各日程区间 | FROM≤TO | — |
| onlyLatestRev | 仅最新改訂 | 默认勾 | — |
| 列表 | 版型多列 | 打数/限界/置場 | 詳細跳编辑 |
| 工具 | 表示/クリア/CSV/Label/picker | — | — |

#### 5.18.5 查询条件字段说明
拠点/担当/适用开始日/手配No/案件/版型No(区间)/名称/種別/新版/客户/代表製品/工程/供应商/置場/棚位 + 詳細(打数·限界打数·最終使用·到着·出庫予定·返却予定·返却日 区间) + onlyLatestRev。FROM≤TO(E10036)。

#### 5.18.6 列表字段说明
版型No/名称/改訂/種別/客户/代表製品/打数/限界打数/最終使用/供应商/到着·出庫·返却各日/返却理由/工程/置場/棚位/新版/刃渡(巾·流れ)/付数/個数/色数/落丁型/置き版/ハンマー/見落とし型/備考 + 詳細。

#### 5.18.7 新增/编辑表单字段说明
编辑页 `/plate-mold`(PA140) 维护版型明细(CRUD,字段同上);详见单页SOP(`待 Explore 核实编辑页操作种别/采番`)。
...
|---|---|---|
| 表示 | 检索 | 检索 |
| クリア | 重置(留onlyLatestRev) | 重置 |
| CSV出力 | 全量导出 | 导出 |
| ラベル発行(N) | 勾选版型标签CSV | 标签 |
| 選択返却(picker) | 选行带回 | picker |
| 詳細(行) | 跳编辑页view | 跳转 |

#### 5.18.10 状态流转说明
onlyLatestRev控制是否仅最新改訂;版型流转(到着→出庫予定→返却予定→返却)。编辑态状态机`待 Explore 核实`。

#### 5.18.11 标准操作步骤
**查版型**：检索(代表製品/客户)→詳細看明细→需要时发行标签/导出。测试点：检索+詳細。

#### 5.18.12 常见问题与处理方法

| 问题 | 原因 | 处理 | 测试关注点 |
|---|---|---|---|
| 找错WMS版型在庫 | 两功能混 | ERP主数据vsWMS库存 | 区分 |
| E10036 | 区间倒置 | 调正 | 区间 |
| E10008 0件 | 条件无匹配 | 放宽 | 空 |
| E10010 标签无选 | 未勾 | 勾行 | 标签 |

#### 5.18.13 用户注意事项
- **ERP 版型主数据 ≠ WMS 版型在庫**(`/wms/plate-mold-stock`)。
- 打数/限界打数=版型寿命管理;onlyLatestRev 默认只看最新改訂。
- **采番 MLD**(MLD+年月+4桁);编辑页操作种别=登录(Register)/改定(Revise,MaxRev+1+前Rev EndDate自动更新)/修正(Edit,仅最新Rev+乐观锁E10023)/删除(管理者论理删+级联删连携受注)/参照。
- **版型登录/改定/修正会自动生成購買受注**(T_Order OrderType=30 購買品 + OrderDetail,118项)并经 PE API 连携外部;删除级联论理删该受注。代表製品CD 引入構成情報(快照)。
...
|---|---|---|---|---|
| PM-01 | 检索 | 多条件 | 命中 | P2 |
| PM-02 | 跳转 | 詳細 | 跳编辑view | P2 |
| PM-03 | 标签 | Label発行 | 标签CSV | P2 |
| PM-04 | 导出 | CSV | 全量 | P2 |
| PM-05 | 边界 | 区间FROM>TO | E10036 | P2 |
| PM-06 | picker | 选行返回 | 带回版型 | P3 |
| PM-07 | 区分 | vs WMS版型在庫 | 不同功能 | P2 |

> 单页 SOP 详见 `sales-msbb-pages/01-S08-版型木型-单页面操作SOP.md`。

---

### 5.19 FSC チェック FscChecklist（FSC 製品化检查表发行）

#### 5.19.1 页面业务目的
**FSC(森林认证)製品的检查表发行页**——按拠点/御見積/客户等检索"FSC 製品(見積計算書 FscProductDiv≠空且≠'0')"的待发行清单，选行+选输出格式，**批量发行 FSC チェックシート(Excel)**；发行时采番 **FSC 管理No** 并回写到見積計算書/御見積计算关联。是**检索+批量发行**页(非 CRUD 编辑)。

#### 5.19.1a 业务前置检查清单（操作前必看）

| 检查项 | 为什么要检查 | 不满足会怎样 | 处理方法 |
|---|---|---|---|
| 拠点(baseCd)必填 | 检索必填 | E10022 | 填拠点 |
| 输出フォーマット必填(发行) | 发行用模板 | E10022 | 选格式 |
| 見積是FSC製品 | 才进清单 | 查不到 | 見積设FscProductDiv |
| 发行先勾行 | 批量目标 | 无对象 | 勾≥1行 |
...
| 字段 | 业务含义 | 说明 |
|---|---|---|
| 御見積書NO `qtnNo`+見積計算書NO `qtnCalcNo` | 来源单 | JOIN来源 |
| FSC製品区分 `fscProductDiv` | 1社外/2社內/3マーク無し | 过滤 |
| FSC管理No `fscManagementNo` | 发行后采番 | 回写 |
| 输出フォーマット `formatName` | 发行模板 | 必填(发行) |
| status | EstimateCalc状态 | 确认/未确认 |
...
|---|---|---|
| 検索 | 拠点空 | E10022 |
| 発行 | formatName空 或 未勾行 | 必填/无目标 |

#### 5.19.1d 完成后检查点与下游验证

| 操作 | 当前页面检查 | 关联检查 | 测试关注点 |
|---|---|---|---|
| 检索 | FSC製品待发行清单 | 仅FscProductDiv≠空≠0 | 过滤 |
| 发行 | 提示发行N件+Excel下载 | EstimateCalc/QuotationCalc(MasterConfirmFlg==1)FscManagementNo回写 | 采番/回写 |

#### 5.19.1e 详细操作场景（SOP）

##### 场景一：批量发行 FSC 检查表
- **业务背景**：FSC 客户要 FSC 检查表。
- **使用数据**：拠点 BASE01、客户 C0001、未发行、选格式。
- **前置条件**：有 FSC 製品見積。
- **详细操作步骤**：1) 进【FSC チェック】;2) 填拠点(必填)/客户/见積条件;3) 勾【未發行】;4) 选输出フォーマット;5)【検索】;6) 勾发行行;7)【発行(N)】确认→采番FSC管理No+生成Excel下载。
- **完成后检查**：发行N件;Excel下载;見積計算書/御見積计算 FscManagementNo回写。
- **异常处理**：拠点/格式空→E10022;件数→E10013。
- **测试用例拆分**：发行(P2)、采番回写(P2)。

##### 场景二：查已发行/下载
- **业务背景**：重下已发行检查表。
- **使用数据**：勾【發行済】。
- **详细操作步骤**：1) 勾發行済→検索;2) 看FSC管理No;3)(下载已发行 download/{fscManagementNo})。
- **完成后检查**：已发行可见+可下载。
- **测试用例拆分**：已发行(P3)。
...
| 角色 | 使用目的 | 常用操作 | 权限说明 |
|---|---|---|---|
| 环境/品质/销售内勤 | 发行FSC检查表 | 检索/发行 | `[Authorize]` |

#### 5.19.3 进入路径
系统菜单 → 销售管理 → FSC チェック（`/fsc-checklist`）。

#### 5.19.4 页面区域说明

| 区域 | 作用 | 关注 | 备注 |
|---|---|---|---|
| 检索区 | 拠点(必填)/担当/作成日/御見積NO/客户/案件/状态(未发行·发行済)+输出フォーマット | 必填拠点+格式 | — |
| 列表 | FSC製品待发行(品名/金额/状态/发行☑/FSC管理No) | 勾发行 | — |
| 工具 | 検索/クリア/発行(N) | — | — |

#### 5.19.5 查询条件字段说明
拠点(必填)/担当/御見積作成日 FROM·TO/御見積NO FROM·TO/客户/案件/未發行(默认)/發行済 + 输出フォーマット(发行必填)。

#### 5.19.6 列表字段说明
rowNo/担当/客户/案件/御見積NO/作成日/顧客品名1·2/数量·単価·金额/status/发行☑/FSC管理No/發行済badge/FSC製品区分。

#### 5.19.7 新增/编辑表单字段说明
无编辑;仅检索+勾选发行。

#### 5.19.8 明细行字段说明
每行=一条 FSC 製品見積(待/已发行)。

#### 5.19.9 按钮功能说明

| 按钮 | 功能 | 前置 | 测试关注点 |
|---|---|---|---|
| 検索 | 7表JOIN查FSC製品 | 拠点 | 检索 |
| 発行(N) | 采番FSC管理No+Excel | 格式+勾行 | 发行 |
| (下载) | download/{fscManagementNo} | 已发行 | 下载 |

#### 5.19.10 状态流转说明
未發行(FscManagementNo空)→发行后采番回写;发行同时更新 QuotationCalc(仅 MasterConfirmFlg==1 确定行)FscManagementNo;受注侧 OrderDetail.FscManagementNo 受注创建时从 Estimate 同步。

#### 5.19.11 标准操作步骤
**发行FSC检查表**：拠点+条件+格式→検索→勾→発行→Excel+采番回写。测试点：发行+回写。

#### 5.19.12 常见问题与处理方法

| 问题 | 原因 | 处理 | 测试关注点 |
|---|---|---|---|
| E10022 拠点/格式 | 必填空 | 补填 | 必填 |
| E10013 | 件数超 | 缩范围 | 截断 |
| 查不到FSC | 見積非FSC製品 | 見積设FscProductDiv | 过滤 |
| 模板缺 | template无 | plain-text fallback(`待确认`) | 模板 |

#### 5.19.13 用户注意事项
- 仅 **FSC 製品**(見積計算書 FscProductDiv≠空且≠'0')进清单。
- 发行采番 **FSC 管理No** 并回写見積計算書 + 御見積计算(确定行 MasterConfirmFlg==1)。
- 受注侧 FSC 管理No 由受注创建时从 Estimate 同步(本页不直接改受注)。

#### 5.19.14 本页面测试点汇总

| 编号 | 测试类型 | 内容 | 预期 | 优先级 |
|---|---|---|---|---|
| FSC-01 | 检索 | FSC製品清单 | 仅FscProductDiv≠空≠0 | P2 |
| FSC-02 | 校验 | 拠点空 | E10022 | P2 |
| FSC-03 | 校验 | 格式空发行 | E10022 | P2 |
| FSC-04 | 发行 | 批量发行 | 采番+Excel | P2 |
| FSC-05 | 回写 | 发行后 | EstimateCalc/QuotationCalc(确定行)回写FSC管理No | P2 |
| FSC-06 | 已发行 | 勾發行済 | 显FSC管理No+可下载 | P3 |

> 单页 SOP 详见 `sales-msbb-pages/01-S09-FSCチェック-单页面操作SOP.md`。
...
| ~~S-02~~ | ~~取引先一覧~~ → **已细化 §5.17** | /business-partner-list | 同上 | BusinessPartner | ✅ 已展开 14 小节(§5.17)+单页SOP(01-S02);11FLG OR/E10030 |
| ~~S-03~~ | ~~信用单 CreditNote~~ → **已细化 §5.13** | /erp/credit-note | Erp/CreditNoteController | CreditNote | ✅ 已展开 14 小节(§5.13)+单页SOP(01-S05);由 WMS RMA 回写产生 |
| ~~S-04~~ | ~~為替レート~~ → **已细化 §5.16** | /erp/fx-rate | Erp/FxRateController | FxRate | ✅ 已展开 14 小节(§5.16)+单页SOP(01-S04);JPY基轴/受注冻结 |
| ~~S-05~~ | ~~OTD 准时交付报表~~ → **已细化 §5.15** | /erp/otd-report | Erp/OtdReportController | Order(聚合) | ✅ 已展开 14 小节(§5.15)+单页SOP(01-S07);准时率KPI |
| ~~S-06~~ | ~~未出荷 Backorder~~ → **已细化 §5.14** | /erp/backorder | Erp/BackorderController | Order | ✅ 已展开 14 小节(§5.14)+单页SOP(01-S06);Close残数/Split分割 |
...
| ~~S-08~~ | ~~版型・木型 / 一覧~~ → **已细化 §5.18** | /plate-mold(+/-list) | Erp/PlateMoldController | PlateMold | ✅ 14小节(§5.18)+SOP(01-S08);采番MLD/自动建購買受注/vs WMS版型在庫 |
| ~~S-09~~ | ~~FSC チェック~~ → **已细化 §5.19** | /fsc-checklist | Erp/FscChecklistController | FscChecklist | ✅ 14小节(§5.19)+SOP(01-S09);批量发行+采番FSC管理No回写 |

> **🎉 M04 销售模块全 19 页(核心10+待补9)SOP 全完成；M04 总册 §5.1~§5.19 全展开。**
