  ...
  <div class="mes-work-center">
    ...
    <el-card shadow="never">
      <div class="table-toolbar">
        ...
        <el-button v-permission="'mes-work-center:edit'" size="small" @click="openCreate">{{ t('新增') }}</el-button>
      ...
      <el-table :data="rows" border stripe size="small" max-height="600" v-loading="loading">
        ...
        <el-table-column :label="t('操作')" width="140" fixed="right">
          <template #default="{ row }">
            <el-button v-permission="'mes-work-center:edit'" link type="primary" size="small" @click="openEdit(row)">{{ t('编辑') }}</el-button>
            <el-button v-permission="'mes-work-center:del'" link type="danger" size="small" @click="doDelete(row)">{{ t('删除') }}</el-button>
          </template>
        </el-table-column>
