<template>
  <div style="padding: 20px">
    ...
    <div style="margin-bottom: 16px">
      <el-button type="primary" :icon="Plus" @click="handleAdd(null)">{{ t('新增根部门') }}</el-button>
      <el-button :icon="Refresh" @click="loadData">{{ t('刷新') }}</el-button>
    </div>

    <el-table :data="tree" row-key="id" border default-expand-all
              :tree-props="{ children: 'children' }">
      <el-table-column prop="deptCode" :label="t('部门编码')" width="160" />
      <el-table-column prop="deptName" :label="t('部门名称')" min-width="200" />
      <el-table-column prop="leaderName" :label="t('部门负责人')" width="160">
        <template #default="{ row }">{{ row.leaderName || '—' }}</template>
      </el-table-column>
      <el-table-column prop="sort" :label="t('排序')" width="80" />
      <el-table-column :label="t('状态')" width="90">
        <template #default="{ row }">
          <el-tag :type="row.enable ? 'success' : 'info'">{{ row.enable ? t('启用') : t('停用') }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column :label="t('操作')" width="280">
        <template #default="{ row }">
          <el-button link type="primary" @click="handleAdd(row)">{{ t('新增子部门') }}</el-button>
          <el-button link type="primary" @click="handleEdit(row)">{{ t('编辑') }}</el-button>
          <el-button link type="primary" @click="handleMove(row)">{{ t('移动') }}</el-button>
    ...
    <el-dialog v-model="dialogVisible" :title="formTitle" width="480px">
      <el-form ref="formRef" :model="form" label-width="90px">
        <el-form-item :label="t('上级部门')">
          <span>{{ parentName || t('（根部门）') }}</span>
        </el-form-item>
        <el-form-item :label="t('部门编码')" prop="deptCode"
                      :rules="[{ required: true, message: t('请输入部门编码') }]">
          <el-input v-model="form.deptCode" :disabled="isEdit" :placeholder="t('建后不可改')" />
        </el-form-item>
        ...
        <el-form-item :label="t('部门负责人')">
          <el-select v-model="form.leaderId" clearable filterable :placeholder="t('选择负责人')" style="width: 100%">
    ...
    <!-- 移动 弹窗 -->
    <el-dialog v-model="moveVisible" :title="t('移动部门')" width="420px">
      <el-form label-width="90px">
        <el-form-item :label="t('移动部门')">
          <span>{{ moveSource?.deptName }}</span>
        </el-form-item>
        <el-form-item :label="t('新上级')">
          <el-tree-select v-model="moveTargetId" :data="tree" clearable check-strictly
                          :props="{ label: 'deptName', value: 'id', children: 'children' }"
                          :placeholder="t('清空 = 设为根部门')" style="width: 100%" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="moveVisible = false">{{ t('取消') }}</el-button>
        <el-button type="primary" @click="handleMoveSubmit">{{ t('确定') }}</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
function handleAdd(parent: DeptTreeNode | null) {
  ...
  formTitle.value = parent ? t('新增子部门') : t('新增根部门')
...
async function handleMoveSubmit() {
  ...
  ElMessage.success(t('已移动'))
  moveVisible.value = false
  loadData()
}

async function handleDelete(row: DeptTreeNode) {
  await ElMessageBox.confirm(t('删除部门「{name}」？', { name: row.deptName }), t('提示'), { type: 'warning' })
