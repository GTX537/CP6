<template>
  <div style="padding: 20px">
    ...
    <el-card style="margin-bottom: 16px">
      <template #header>表元数据</template>
      <el-form :model="table" inline label-width="90px">
        <el-form-item label="实体名"><el-input v-model="table.entityName" placeholder="Demo" /></el-form-item>
        <el-form-item label="模块"><el-input v-model="table.module" placeholder="Erp" /></el-form-item>
        <el-form-item label="表名"><el-input v-model="table.tableName" placeholder="Erp_Demo" /></el-form-item>
        <el-form-item label="资源键"><el-input v-model="table.resourceKey" placeholder="demo" /></el-form-item>
        <el-form-item label="采番键"><el-input v-model="table.seqBizKey" placeholder="DEMO（可空）" /></el-form-item>
        <el-form-item label="采番字段"><el-input v-model="table.codeField" placeholder="DemoNo（可空）" /></el-form-item>
        <el-form-item label="路由"><el-input v-model="table.routePath" placeholder="api/erp/demo" /></el-form-item>
      </el-form>
    </el-card>
    ...
    <el-card style="margin-bottom: 16px">
      ...
      <el-table :data="columns" border size="small">
        <el-table-column label="属性名" width="160">
          <template #default="{ row }"><el-input v-model="row.name" /></template>
        </el-table-column>
        <el-table-column label="类型" width="140">
          <template #default="{ row }">
            <el-select v-model="row.clrType">
              <el-option v-for="t in clrTypes" :key="t" :label="t" :value="t" />
            </el-select>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="70">
          <template #default="{ $index }"><el-button link type="danger" @click="columns.splice($index, 1)">删</el-button></template>
        </el-table-column>
      </el-table>
    </el-card>

    <div style="margin-bottom: 16px">
      <el-button type="primary" @click="preview">预览生成</el-button>
      <el-button @click="save">保存元数据</el-button>
    </div>

    <el-card v-if="Object.keys(files).length">
      <template #header>生成产物预览</template>
      <el-tabs>
