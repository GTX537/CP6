  ...
  <div class="ap-aging">
    ...
    <el-card shadow="never">
      <div class="table-toolbar">
        ...
        <el-input v-model="supplierId" size="small" style="width: 160px" :placeholder="t('供应商')" clearable @change="reload" />
      ...
      <el-table :data="rows" border stripe size="small" max-height="620" v-loading="loading">
        <el-table-column prop="supplierId" :label="t('供应商')" min-width="140" />
        ...
        <el-table-column prop="total" :label="t('未付合计')" width="130" align="right" />
      </el-table>
    </el-card>
  </div>
</template>
...
import { apInvoiceApi } from '@/api/fin/fin'
import type { ApAgingRow, ApReconcileResult } from '@/types/fin/fin'

const { t } = useI18n()
const rows = ref<ApAgingRow[]>([])
const recon = ref<ApReconcileResult | null>(null)
...
async function reload() {
  ...
  try {
    const [a, r] = await Promise.all([
      apInvoiceApi.aging(asOf.value, supplierId.value || undefined),
      apInvoiceApi.reconcile(),
    ])
    rows.value = a?.data || []
