<template>
  <div class="todo-center">
    <div class="page-header"><h2>待办中心</h2></div>

    <el-card shadow="never" class="table-card">
      <div class="table-toolbar">
        <el-tag size="small">共 {{ rows.length }} 条待办</el-tag>
      ...
      <el-table :data="rows" border stripe size="small" max-height="620" v-loading="loading">
        <el-table-column prop="flowKey" label="流程" width="160" />
        <el-table-column prop="nodeId" label="当前节点" width="140" />
        <el-table-column label="发起人" min-width="180" show-overflow-tooltip>
        ...
        <el-table-column label="操作" width="120" fixed="right">
          <template #default="{ row }">
            <el-button type="primary" link size="small" @click="openTodo(row)">办理</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-empty v-if="!rows.length && !loading" description="暂无待办" :image-size="80" />
    </el-card>

    <el-dialog v-model="dialog.visible" title="办理待办" width="560px">
      <div v-loading="dialog.loading">
        ...
        <el-input v-model="dialog.comment" type="textarea" :rows="3" maxlength="500" show-word-limit placeholder="可填写审批意见" />
      </div>
      <template #footer>
        <el-button @click="dialog.visible = false">取消</el-button>
        <el-button type="danger" :loading="dialog.acting" @click="act(false)">驳回</el-button>
        <el-button type="primary" :loading="dialog.acting" @click="act(true)">同意</el-button>
      </template>
    </el-dialog>
  </div>
</template>
