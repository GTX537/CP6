...
/// <summary>
/// M-PUR Task 3 权限拒绝用例（403 断言）——覆盖真相源 §三高危 7 键对应端点。
/// 口径照仓内既有先例 <c>PermissionChainIntegrationTests</c> / <c>FourGranularityIntegrationTests</c>：
/// 走**真实后端聚合链**（PermissionAggregator → CurrentPermissionContext → PermissionService，InMemory DB），
/// 一个**已登录但无该操作权**的用户经生产 <see cref="RequirePermissionAttribute"/> 请求高危端点 → 断言 403。
///
/// 关于「无认证 401」口径取舍：本仓 [Authorize] 认证层（401）在 HTTP 传输层，进程内 <c>RequirePermission</c>
/// 过滤器不经它；且 <c>CurrentPermissionContext.GetAsync</c> 对无 Identity.Name 会 throw "未登录"（非返回 401）。
/// 故遵循 brief 明列的可行口径「**无权限身份 → 403**」——与既有两先例逐字一致（授一无关键放行 + 目标键 403）。
///
/// 高危 7 键（真相源 §三）以**测试内独立字面量**声明；并经反射交叉核验：每个 (控制器.方法) 确实携带该 (menu,action)
/// [RequirePermission]——若生产端点改名/改键，本用例的 403 oracle 亦随之破（不会静默漂移）。
/// </summary>
public class PurHighRiskDenialTests
{
    /// <summary>真相源 §三高危 7 键：(控制器名.方法名, menu-key, action)。独立字面量，非引用生产常量。</summary>
    private static readonly (string endpoint, string menu, string action)[] HighRiskKeys =
    {
        ("GoodsReceiptController.Confirm",     "pur-gr",          "add"),      // 确认收货=不可逆入库
        ("ThreeWayMatchController.Match",      "pur-match",       "add"),      // 三单匹配容差内自动建 AP
        ("ThreeWayMatchController.Release",    "pur-match",       "release"),  // 超容差人工放行建 AP
        ("PurchaseRequestController.Convert",  "pur-pr",          "convert"),  // PR→PO 转单建采购承诺
        ("RfqController.Convert",              "pur-rfq",         "convert"),  // RFQ 选中报价转 PO
        ("SubcontractController.Issue",        "pur-subcontract", "issue"),    // 发支給材=委托 WMS 实物出库
        ("SubcontractController.FinishedCost", "pur-subcontract", "cost"),     // 外注成品成本入账
    };

    /// <summary>无关的「良性放行键」——证明链非「全盘拒绝」假绿（有此键的请求必须放行）。</summary>
    private const string BenignMenu = "pur-po";
    ...
    /// <summary>登录用户 "u"（RoleId=1），**仅授 pur-po:add** 一个良性键——高危 7 键一概未授。</summary>
    private static ServiceProvider BuildProvider()
    {
        var db = NewDb();
        db.Sys_Users.Add(new Sys_User { Id = Guid.NewGuid(), UserName = "u", Password = "x", RoleId = 1 });
        // 良性键锚定菜单（pur-po 有 MenuKey 才能 join 出 ActionKeys）
        db.Sys_Menus.Add(new Sys_Menu { MenuId = 702, MenuName = "采购订单", MenuKey = BenignMenu });
        db.Sys_RoleActions.Add(new Sys_RoleAction { Id = Guid.NewGuid(), RoleId = 1, MenuId = 702, ActionCode = BenignAction });
        db.SaveChanges();
    ...
    private static (string menu, string action)? ReadEndpointPermission(string endpoint)
    {
        ...
        var ctrl = typeof(PurchaseOrderController).Assembly.GetTypes()
            .FirstOrDefault(t => t.Namespace == "CP6.WebApi.Controllers.Pur" && t.Name == parts[0]);
    ...
    {
        // 正控：有 pur-po:add 的用户请求该键 → 放行（证明链非全盘拒绝，403 用例才有意义）。
    ...
    public async Task UnauthorizedUser_Is403_OnEveryHighRiskEndpoint()
    {
        ...
        Assert.Equal(7, HighRiskKeys.Length);   // 收口：覆盖真相源 §三全 7 高危键
    }
}
