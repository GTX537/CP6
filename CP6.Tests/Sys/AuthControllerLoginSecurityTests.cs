...
public class AuthControllerLoginSecurityTests
{
    ...
    private static (AuthController ctl, CP6Context db, Sys_User user) Make(string plainPassword, int maxFailed = 3)
    {
        ...
        var ctl = new AuthController(db, new ConfigurationBuilder().Build(), new FakePermCtx(), new TenantContext(), hasher, policy, login, audit, refresh, blacklist, cookies, opt, new FakeTenantSsoConfigService(), new FakeSsoService(), twoFa, pending);
