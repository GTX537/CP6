...
/// EnterNodeAsync 退化为 _handlers[node.Type].OnEnterAsync(ctx)。改库不落盘（调用方统一 SaveChanges）。
/// public 以供 WebApi DI 注入（FlowEngine 公有构造接 IEnumerable&lt;INodeHandler&gt;）；实现类仍 internal。</summary>
