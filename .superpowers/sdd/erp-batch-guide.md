# ERP 模块迁移批次指南(feat/ui-migrate-erp)

适用于 Milestone C 首模块 ERP 的每个迁移批次。实现者在派发消息中收到本批页面清单,其余规则全在本文件。
...
2. i18n: 所有原 t() 词条保留;CpFilterBar 按钮用 `:filter-labels`(search→common搜索词条(按原页grep确认,ERP多用t键不同于wms.common.*——以原页现有键为准,不臆造)、reset→同上,与样板一致);不臆造 Sys_Langs 词条。
...
3. 本批一个 commit: `refactor(ui): ERP 迁移批次<N>——<页1>/<页2>/...` + trailer `Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>`。
