# WMS 模块迁移批次指南(feat/ui-migrate-wms)

适用于 Milestone C 首模块 WMS 的每个迁移批次。实现者在派发消息中收到本批页面清单,其余规则全在本文件。
...
2. i18n: 所有原 t() 词条保留;CpFilterBar 按钮用 `:filter-labels`(search→wms.common.search、reset→wms.common.clear,与样板一致);不臆造 Sys_Langs 词条。
...
3. 本批一个 commit: `refactor(ui): WMS 迁移批次<N>——<页1>/<页2>/...` + trailer `Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>`。
