...
public static class I18nBackendMsgSeed
{
    public static readonly Sys_Lang[] Items = new[]
    {
        ...
        new Sys_Lang { LangKey = "租户不存在", ZhCN = "租户不存在", ZhTW = "租戶不存在", En = "Tenant does not exist", Ja = "テナントが存在しません", Ko = "테넌트가 존재하지 않습니다" },
        new Sys_Lang { LangKey = "租户已停用", ZhCN = "租户已停用", ZhTW = "租戶已停用", En = "Tenant is disabled", Ja = "テナントが無効化されています", Ko = "테넌트가 비활성화되었습니다" },
        new Sys_Lang { LangKey = "该用户名存在于多个租户，请提供租户编码", ZhCN = "该用户名存在于多个租户，请提供租户编码", ZhTW = "該使用者名稱存在於多個租戶，請提供租戶編碼", En = "This username exists in multiple tenants; please provide a tenant code", Ja = "このユーザー名は複数のテナントに存在します。テナントコードを指定してください", Ko = "이 사용자 이름은 여러 테넌트에 존재합니다. 테넌트 코드를 입력하세요" },
