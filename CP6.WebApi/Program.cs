...
builder.Services.AddScoped<CP6.Core.Services.Wf.INodeHandler, CP6.Core.Services.Wf.EndNodeHandler>();        // WFS T4 节点处理器：结束（ParallelSplit/Join 在 T5 注册）
...
builder.Services.AddSingleton<CP6.Core.Services.Fin.IDepreciationCalculator, CP6.Core.Services.Fin.DepreciationCalculator>(); // A3 §3.1 折旧引擎纯函数四法
builder.Services.AddScoped<CP6.Core.Services.Fin.IAssetDepreciationService, CP6.Core.Services.Fin.AssetDepreciationService>(); // A3 §3.2 资产折旧服务（三路：手动/Worker/结账钩子）
...
builder.Services.AddScoped<CP6.Core.Services.Pur.Contracts.IWmsIssueService, CP6.Core.Services.Pur.Contracts.WmsIssueServiceAdapter>(); // P2-D3 WMS 出库适配器（真实；外注支給材委托 IStockMovementService OUT 扣减，RelatedType=SUBCONTRACT，库存不足→E-PUR-080）
builder.Services.AddScoped<CP6.Core.Services.Pur.Contracts.IFinCostService, CP6.Core.Services.Pur.Contracts.FinCostServiceAdapter>(); // P2-D2 财务成本入账适配器（真实；外注成品成本经 IAutoVoucherEngine 规则 Subcontract.CostPosted 借FG贷INVENTORY，零重复应付）
...
using (var scope = app.Services.CreateScope())
{
    ...
    // PermissionService.HasActionAsync 无 admin 旁路：贴 [RequirePermission] 的端点必须在此 seed+授权，否则 admin 也 403。幂等。
    {
        ...
        var finActions = new (int MenuId, string Code, string Name)[]
        {
            ...
            // A3 §7 固定资产 4 菜单（615~618）操作权限点（view 为读端点权限点，控制器 GET 强校验）
            (615, "view", "查看"), (615, "add", "新建"), (615, "edit", "编辑"), (615, "delete", "删除"),
