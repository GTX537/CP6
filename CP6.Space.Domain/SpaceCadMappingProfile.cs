...
public sealed class SpaceCadMappingProfile : SpaceTenantEntity
{
    private SpaceCadMappingProfile()
    {
    }
    ...
    public static SpaceCadMappingProfile Create(Guid tenantId, string name)
    {
        var profile = new SpaceCadMappingProfile();
    ...
    public void Advance(string name, int version)
    {
        if (version != checked(CurrentVersion + 1))
        {
            throw new InvalidOperationException(
                "CAD mapping profile versions must advance by exactly one.");
        }
    ...
    private void SetName(string name)
    {
        ...
        if (normalized.Length is < 1 or > 200)
        {
            throw new ArgumentException(
                "CAD mapping profile name must contain 1 to 200 characters.",
                nameof(name));
        }
...
public sealed class SpaceCadMappingProfileVersion : SpaceTenantEntity
{
    private SpaceCadMappingProfileVersion()
    {
    }
    ...
    public static SpaceCadMappingProfileVersion Create(
        Guid tenantId,
        Guid profileId,
        int version,
        ...
        int? basedOnVersion)
    {
        ...
        {
            throw new ArgumentException(
                "Definition hash must be SHA-256.",
        ...
        {
            throw new ArgumentException(
                "Base profile cannot be empty.",
                nameof(basedOnProfileId));
        }
        ...
        var item = new SpaceCadMappingProfileVersion
        {
            ProfileId = profileId,
            Version = version,
